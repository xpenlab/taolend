...
async function main() {
    try {
        ...
        .option("address", {
            type: "string",
            describe: "Treasury SS58 address",
            demandOption: true,
        })
        .strict()
        .parse();
        ...
        console.log("Admin set treasury coldkey:", argv.address);
        let tx = await lendingPool.setTreasuryColdkey(ss58ToPublicKey(argv.address));
        console.log(`Admin set treasury coldkey successfully, transaction hash: ${tx.hash}`);
