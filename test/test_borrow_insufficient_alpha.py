#!/usr/bin/env python3
"""
Test Case TC06: Insufficient Borrower ALPHA
Objective: Verify borrow fails when borrower lacks sufficient ALPHA
Tests: Line 331 - require(userAlphaBalance[msg.sender][_netuid] >= _alphaAmount, "low alpha")

Strategy: Attempt borrow with collateral amount exceeding borrower's ALPHA balance
...
def main():
    print_section("Test Case TC06: Insufficient Borrower ALPHA")
    print(f"{CYAN}Objective:{NC} Verify borrow fails when borrower lacks sufficient ALPHA")
    print(f"{CYAN}Strategy:{NC} Attempt borrow with collateral amount exceeding ALPHA balance")
    ...
    # Test parameters
    netuid = offer['netuid']
    tao_amount = 50 * 10**9  # 50 TAO in RAO (valid amount)
    alpha_amount = 1000 * 10**9  # 1000 ALPHA in RAO (exceeds borrower's 500 ALPHA balance)
    ...
    # Check borrower has INSUFFICIENT ALPHA (this is the test condition)
    ...
    print_info(f"Required collateral: {alpha_amount / 1e9:.2f} ALPHA")
    if borrower_alpha >= alpha_amount:
        print_error("SETUP ERROR: Borrower has sufficient ALPHA")
        print_error(f"  Required: < {alpha_amount / 1e9:.2f} ALPHA")
        print_error(f"  Available: {borrower_alpha / 1e9:.2f} ALPHA")
        print_error("This test requires borrower to have insufficient ALPHA")
        sys.exit(1)
    print_success(f"✓ Borrower has insufficient ALPHA: {borrower_alpha / 1e9:.2f} < {alpha_amount / 1e9:.2f} ALPHA (as required)")
    ...
    print(f"  {CYAN}Reason:{NC} Borrower ALPHA balance insufficient for collateral")
    ...
    print_info(f"Borrower ALPHA balance: {borrower_alpha / 1e9:.2f} ALPHA (insufficient)")
    ...
    try:
        ...
        if "low alpha" in error_msg.lower() or "alpha" in error_msg.lower():
    ...
    # Verify revert reason contains "low alpha"
    ...
    print_success("TC06: Insufficient Borrower ALPHA")
    print_success(f"Transaction correctly reverted with 'low alpha'")
    print_success("All state validations passed")
    print_success("No unexpected state changes detected")

    print(f"\n{CYAN}Summary:{NC}")
    print(f"  - Borrow with insufficient ALPHA collateral correctly fails")
    print(f"  - Borrower ALPHA balance validation working correctly")
    print(f"  - Contract protects lenders by requiring sufficient collateral")
