#!/usr/bin/env python3
"""
Test Case TC07: Insufficient Lender TAO
Objective: Verify borrow fails when lender lacks sufficient TAO
Tests: Line 886 - require(userAlphaBalance[_offer.lender][0] >= _taoAmount, "low lender tao")

Strategy: Attempt borrow with loan amount exceeding lender's TAO balance
...
def main():
    print_section("Test Case TC07: Insufficient Lender TAO")
    print(f"{CYAN}Objective:{NC} Verify borrow fails when lender lacks sufficient TAO")
    print(f"{CYAN}Strategy:{NC} Attempt borrow with loan amount exceeding lender's TAO balance")
    ...
    # Test parameters
    netuid = offer['netuid']
    tao_amount = 150 * 10**9  # 150 TAO in RAO (exceeds lender's 100 TAO balance)
    alpha_amount = 750 * 10**9  # 750 ALPHA in RAO (sufficient collateral for 150 TAO)
    ...
    # Check lender has INSUFFICIENT TAO (this is the test condition)
    ...
    print_info(f"Required loan amount: {tao_amount / 1e9:.2f} TAO")
    if lender_tao >= tao_amount:
        print_error("SETUP ERROR: Lender has sufficient TAO")
        print_error(f"  Required: < {tao_amount / 1e9:.2f} TAO")
        print_error(f"  Available: {lender_tao / 1e9:.2f} TAO")
        print_error("This test requires lender to have insufficient TAO")
        sys.exit(1)
    print_success(f"✓ Lender has insufficient TAO: {lender_tao / 1e9:.2f} < {tao_amount / 1e9:.2f} TAO (as required)")

    # Check borrower has SUFFICIENT ALPHA (required for this test)
    ...
    print(f"  {CYAN}Reason:{NC} Lender TAO balance insufficient for loan")
    ...
    print_info(f"Lender TAO balance: {lender_tao / 1e9:.2f} TAO (insufficient)")
    ...
    try:
        ...
        if "low lender tao" in error_msg.lower() or "lender" in error_msg.lower():
            print_success(f"✓ Revert reason contains 'low lender tao'")
    ...
    # Verify revert reason contains "low lender tao"
    if revert_reason and "low lender tao" in revert_reason.lower():
        print_success("✓ Revert reason confirmed: 'low lender tao'")
    elif revert_reason:
    ...
    print_success("TC07: Insufficient Lender TAO")
    print_success(f"Transaction correctly reverted with 'low lender tao'")
    print_success("All state validations passed")
    print_success("No unexpected state changes detected")

    print(f"\n{CYAN}Summary:{NC}")
    print(f"  - Borrow with insufficient lender TAO correctly fails")
    print(f"  - Lender TAO balance validation working correctly")
    print(f"  - Contract protects against over-lending")
