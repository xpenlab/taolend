#!/usr/bin/env python3
"""
Test Case TC05: Loan Amount Too Small
Objective: Verify borrow fails when loan amount is below minimum
Tests: Line 330 - require(_taoAmount >= MIN_LOAN_AMOUNT, "loan too small")

Strategy: Attempt borrow with amount below MIN_LOAN_AMOUNT (0.1 TAO)
Expected: Transaction reverts with "loan too small"
"""
...
def main():
    print_section("Test Case TC05: Loan Amount Too Small")
    print(f"{CYAN}Objective:{NC} Verify borrow fails when loan amount is below minimum")
    print(f"{CYAN}Strategy:{NC} Attempt borrow with amount below MIN_LOAN_AMOUNT (0.1 TAO)")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'loan too small'\n")
    ...
    # Test parameters
    netuid = offer['netuid']
    tao_amount = 5 * 10**7  # 0.05 TAO in RAO (below MIN_LOAN_AMOUNT = 0.1 TAO)
    alpha_amount = 25 * 10**9  # 25 ALPHA in RAO (sufficient collateral)
    ...
    if borrower_alpha < alpha_amount:
        print_warning(f"⚠ Borrower has insufficient ALPHA (will fail later in test)")
    else:
    ...
    print(f"  {RED}Revert:{NC} 'loan too small'")
    print(f"  {CYAN}Reason:{NC} Loan amount is below MIN_LOAN_AMOUNT (0.1 TAO)")
    ...
    print_info(f"Attempting to borrow {tao_amount / 1e9:.4f} TAO with {alpha_amount / 1e9:.2f} ALPHA collateral...")
    ...
    try:
        ...
        if "loan too small" in error_msg.lower() or "too small" in error_msg.lower():
            print_success(f"✓ Revert reason contains 'loan too small'")
    ...
    # Verify revert reason contains "loan too small"
    if revert_reason and ("loan too small" in revert_reason.lower() or "too small" in revert_reason.lower()):
        print_success("✓ Revert reason confirmed: 'loan too small'")
    elif revert_reason:
    ...
    print_success("TC05: Loan Amount Too Small")
    print_success(f"Transaction correctly reverted with 'loan too small'")
    print_success("All state validations passed")
    print_success("No unexpected state changes detected")

    print(f"\n{CYAN}Summary:{NC}")
    print(f"  - Borrow with amount below MIN_LOAN_AMOUNT (0.1 TAO) correctly fails")
    print(f"  - Loan amount validation working correctly")
    print(f"  - Contract protects against dust loans")
