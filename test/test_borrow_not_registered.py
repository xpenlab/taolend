#!/usr/bin/env python3
"""
Test Case TC01: Borrower Not Registered
Objective: Verify borrow fails when borrower is not registered
...
def main():
    print_section("Test Case TC01: Borrower Not Registered")
    print(f"{CYAN}Objective:{NC} Verify borrow fails when borrower is not registered")
    print(f"{CYAN}Strategy:{NC} Attempt borrow with unregistered borrower")
    ...
    # Check BORROWER1 is NOT registered (this is the test condition)
    borrower_registered = contract.functions.registeredUser(borrower_address).call()
    if borrower_registered:
        print_error("SETUP ERROR: BORROWER1 is already registered")
        print_error("This test requires BORROWER1 to be unregistered")
        print_info("Please use a different unregistered account for this test")
        sys.exit(1)
    print_success(f"✓ BORROWER1 NOT registered (as required): {borrower_address}")
    ...
    # Check borrower has ALPHA (even though not registered, for completeness)
    ...
    if borrower_alpha < alpha_amount:
        print_warning(f"⚠ Borrower has insufficient ALPHA (but test should fail at registration check first)")
    ...
    print(f"  {CYAN}Reason:{NC} BORROWER1 is not registered (onlyRegistered modifier)")
    ...
    print_info(f"Borrower: {borrower_address} (NOT registered)")
    ...
    print_success("TC01: Borrower Not Registered")
    ...
    print(f"\n{CYAN}Summary:{NC}")
    print(f"  - Unregistered borrower cannot call borrow()")
