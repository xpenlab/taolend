#!/usr/bin/env python3
"""
Test Case TC04: cancel() - Initiator Not Registered
Objective: Verify cancel() fails when initiator is not registered
...
def main():
    print_section("Test Case TC04: cancel() - Initiator Not Registered")
    print(f"{CYAN}Objective:{NC} Verify cancel() fails when initiator is not registered")
    print(f"{CYAN}Strategy:{NC} Attempt batch cancel with unregistered user")
    ...
    # Query nonce (should be 0 or default)
    nonce_before = contract.functions.lenderNonce(unregistered_address).call()
    print_info(f"lenderNonce[{unregistered_address}] = {nonce_before}")
    ...
    # ========================================================================
    print_section("Step 4: Execute cancel() - Should FAIL")
    ...
    print_info(f"Attempting to cancel all offers with unregistered user...")
    ...
    # Query nonce
    nonce_after = contract.functions.lenderNonce(unregistered_address).call()
    print_info(f"lenderNonce[{unregistered_address}] = {nonce_after}")
    ...
    print_info(f"Current nonce: {nonce_after} (should be unchanged)")
    ...
    # Check 1: lenderNonce unchanged
    if nonce_after == nonce_before:
        print_success(f"✓ lenderNonce unchanged: {nonce_before} → {nonce_after}")
    else:
        print_error(f"✗ lenderNonce changed: {nonce_before} → {nonce_after}")
        all_checks_passed = False

    # Check 2: Contract state unchanged
