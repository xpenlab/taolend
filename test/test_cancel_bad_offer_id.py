#!/usr/bin/env python3
"""
Test Case TC08: Cancel with Invalid Offer ID
Objective: Verify cancel(Offer) reverts when offerId doesn't match calculated value
Tests: verifyOffer modifier - require(_offer.offerId == LoanLib.calculateOfferId(_offer), "bad offer id")

Strategy: Create valid offer, tamper with offerId, attempt to cancel
Expected: Transaction reverts with "bad offer id" error
Note: offerId is keccak256 hash of offer data, tampering breaks integrity check
"""
...
def tamper_offer_id(offer_id_hex):
    """Tamper with offer ID by flipping one byte"""
    offer_id_bytes = bytes.fromhex(offer_id_hex[2:])

    # Flip one byte in the middle
    tampered = bytearray(offer_id_bytes)
    tampered[15] ^= 0xFF  # Flip byte at position 15

    return '0x' + tampered.hex()

def main():
    print_section("Test Case TC08: Cancel with Invalid Offer ID")
    print(f"{CYAN}Objective:{NC} Verify cancel(Offer) reverts when offerId doesn't match calculated value")
    print(f"{CYAN}Strategy:{NC} Create valid offer, tamper with offerId, attempt to cancel")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'bad offer id'\n")
    ...
    # Step 0: Verify Setup and Tamper Offer ID
    print_section("Step 0: Verify Setup and Tamper Offer ID")
    ...
    # Tamper with offer ID
    original_offer_id = offer['offerId']
    tampered_offer_id = tamper_offer_id(original_offer_id)

    print_info(f"Original offerId: {original_offer_id}")
    print_info(f"Tampered offerId: {tampered_offer_id}")
    print_warning("Offer ID has been tampered (byte 15 flipped)")

    # Create tampered offer
    tampered_offer = offer.copy()
    tampered_offer['offerId'] = tampered_offer_id

    # Step 1: Read Initial Contract State
    print_section("Step 1: Read Initial Contract State")

    # Use ORIGINAL offer ID for state queries (tampered ID doesn't exist)
    ...
    print_section("Step 4: Execute cancel(Offer) with TAMPERED offerId")

    print(f"\n{BOLD}{RED}Expected Result:{NC}")
    print(f"  {RED}Revert:{NC} Transaction reverts (status=0)")
    print(f"  {YELLOW}Reason:{NC} 'bad offer id'")
    print(f"  {CYAN}State Changes:{NC} None (only gas consumed)\n")

    # Convert tampered offer to tuple for contract call
    offer_tuple = (
        bytes.fromhex(tampered_offer['offerId'][2:]),  # TAMPERED offer ID
        Web3.to_checksum_address(tampered_offer['lender']),
        tampered_offer['netuid'],
        tampered_offer['nonce'],
        tampered_offer['expire'],
        tampered_offer['maxTaoAmount'],
        tampered_offer['maxAlphaPrice'],
        tampered_offer['dailyInterestRate'],
        bytes.fromhex(tampered_offer['signature'][2:])
    )

    print_info(f"Attempting to cancel with tampered offerId...")
    ...
    try:
        ...
        if "bad offer id" in error_str.lower() or "offer id" in error_str.lower():
            reverted = True
            print_success("✓ Transaction reverted with expected reason (bad offer id)")
        else:
    ...
    print_success("Cancel operation correctly reverted for invalid offerId")
    print_success("Verification: offerId integrity check working correctly")
