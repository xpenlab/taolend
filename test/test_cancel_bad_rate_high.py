#!/usr/bin/env python3
"""
Test Case TC07b: Cancel with Daily Rate Too High
Objective: Verify cancel(Offer) reverts when dailyInterestRate > MAX_DAY_RATE
Tests: verifyOffer modifier - require(_offer.dailyInterestRate <= MAX_DAY_RATE, "bad rate")

Strategy: Create valid offer, set dailyInterestRate above 30000000 (3%), attempt to cancel
Expected: Transaction reverts with "bad rate" error
Note: MAX_DAY_RATE = 30000000 (3% daily)
"""
...
def main():
    print_section("Test Case TC07b: Cancel with Daily Rate Too High")
    print(f"{CYAN}Objective:{NC} Verify cancel(Offer) reverts when dailyInterestRate > MAX_DAY_RATE")
    print(f"{CYAN}Strategy:{NC} Create valid offer, set dailyInterestRate to 30000001, attempt to cancel")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'bad rate'\n")
    ...
    # Step 0: Verify Setup and Modify Daily Rate
    print_section("Step 0: Verify Setup and Modify Daily Rate to Invalid Value")
    ...
    # Modify dailyInterestRate to invalid value (just above MAX_DAY_RATE)
    original_rate = offer['dailyInterestRate']
    invalid_rate = 30000001  # Just above MAX_DAY_RATE (30000000 = 3%)

    print_info(f"Original dailyInterestRate: {original_rate / 10000}% ({original_rate})")
    print_info(f"Modified dailyInterestRate: {invalid_rate / 10000}% ({invalid_rate})")
    print_info(f"MAX_DAY_RATE: 3% (30000000)")
    print_warning(f"Modified rate > MAX_DAY_RATE (validation limit)")

    # Create modified offer
    modified_offer = offer.copy()
    modified_offer['dailyInterestRate'] = invalid_rate
    ...
    print_section("Step 4: Execute cancel(Offer) with INVALID dailyInterestRate (too high)")

    print(f"\n{BOLD}{RED}Expected Result:{NC}")
    print(f"  {RED}Revert:{NC} Transaction reverts (status=0)")
    print(f"  {YELLOW}Reason:{NC} 'bad rate'")
    print(f"  {CYAN}State Changes:{NC} None (only gas consumed)\n")

    # Convert modified offer to tuple for contract call
    offer_tuple = (
        bytes.fromhex(modified_offer['offerId'][2:]),
        Web3.to_checksum_address(modified_offer['lender']),
        modified_offer['netuid'],
        modified_offer['nonce'],
        modified_offer['expire'],
        modified_offer['maxTaoAmount'],
        modified_offer['maxAlphaPrice'],
        modified_offer['dailyInterestRate'],  # INVALID value (> MAX_DAY_RATE)
        bytes.fromhex(modified_offer['signature'][2:])
    )

    print_info(f"Attempting to cancel with invalid dailyInterestRate ({invalid_rate / 10000}%)...")
    ...
    try:
        ...
        if "bad rate" in error_str.lower() or "rate" in error_str.lower():
            reverted = True
            print_success("✓ Transaction reverted with expected reason (bad rate)")
        else:
    ...
    print_success("Cancel operation correctly reverted for dailyInterestRate too high")
    print_success("Verification: MAX_DAY_RATE check (<= 30000000) working correctly")
