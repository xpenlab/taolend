#!/usr/bin/env python3
"""
Test Case TC07a: Cancel with Daily Rate Too Low
Objective: Verify cancel(Offer) reverts when dailyInterestRate < MIN_DAY_RATE
Tests: verifyOffer modifier - require(_offer.dailyInterestRate >= MIN_DAY_RATE, "bad rate")

Strategy: Create valid offer, set dailyInterestRate below 1000000 (0.01%), attempt to cancel
Expected: Transaction reverts with "bad rate" error
Note: MIN_DAY_RATE = 1000000 (0.01% daily)
"""
...
def main():
    print_section("Test Case TC07a: Cancel with Daily Rate Too Low")
    print(f"{CYAN}Objective:{NC} Verify cancel(Offer) reverts when dailyInterestRate < MIN_DAY_RATE")
    print(f"{CYAN}Strategy:{NC} Create valid offer, set dailyInterestRate to 999999, attempt to cancel")
    ...
    # Modify dailyInterestRate to invalid value (just below MIN_DAY_RATE)
    original_rate = offer['dailyInterestRate']
    invalid_rate = 999999  # Just below MIN_DAY_RATE (1000000 = 0.01%)
    ...
    print_info(f"MIN_DAY_RATE: 0.01% (1000000)")
    print_warning(f"Modified rate < MIN_DAY_RATE (validation limit)")
    ...
    print_section("Step 4: Execute cancel(Offer) with INVALID dailyInterestRate (too low)")
    ...
    offer_tuple = (
        ...
        modified_offer['dailyInterestRate'],  # INVALID value (< MIN_DAY_RATE)
    ...
    print_success("Cancel operation correctly reverted for dailyInterestRate too low")
    print_success("Verification: MIN_DAY_RATE check (>= 1000000) working correctly")
