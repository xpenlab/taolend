#!/usr/bin/env python3
"""
Test Case TC09: Cancel with Invalid Signature
Objective: Verify cancel(Offer) reverts when signature is invalid
Tests: verifyOffer modifier - require(LoanLib.verifyOfferSignature(_offer), "bad signature")

Strategy: Create valid offer, corrupt signature, attempt to cancel
Expected: Transaction reverts with "bad signature" error
"""
...
def corrupt_signature(signature_hex):
    """Corrupt a signature by flipping some bytes"""
    sig_bytes = bytes.fromhex(signature_hex[2:])

    # Flip some bytes in the middle of the signature
    corrupted = bytearray(sig_bytes)
    corrupted[10] ^= 0xFF  # Flip byte at position 10
    corrupted[20] ^= 0xFF  # Flip byte at position 20
    corrupted[30] ^= 0xFF  # Flip byte at position 30

    return '0x' + corrupted.hex()

def main():
    print_section("Test Case TC09: Cancel with Invalid Signature")
    print(f"{CYAN}Objective:{NC} Verify cancel(Offer) reverts when signature is invalid")
    print(f"{CYAN}Strategy:{NC} Create valid offer, corrupt signature, attempt to cancel")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'bad signature'\n")
    ...
    # Step 0: Verify Setup and Corrupt Signature
    print_section("Step 0: Verify Setup and Corrupt Signature")
    ...
    # Corrupt signature
    original_signature = offer['signature']
    corrupted_signature = corrupt_signature(original_signature)

    print_info(f"Original signature: {original_signature[:20]}...")
    print_info(f"Corrupted signature: {corrupted_signature[:20]}...")
    print_warning("Signature has been corrupted by flipping bytes 10, 20, 30")

    # Create corrupted offer
    corrupted_offer = offer.copy()
    corrupted_offer['signature'] = corrupted_signature
    ...
    print_section("Step 4: Execute cancel(Offer) with CORRUPTED signature")

    print(f"\n{BOLD}{RED}Expected Result:{NC}")
    print(f"  {RED}Revert:{NC} Transaction reverts (status=0)")
    print(f"  {YELLOW}Reason:{NC} 'bad signature'")
    print(f"  {CYAN}State Changes:{NC} None (only gas consumed)\n")

    # Convert corrupted offer to tuple for contract call
    offer_tuple = (
        bytes.fromhex(corrupted_offer['offerId'][2:]),
        Web3.to_checksum_address(corrupted_offer['lender']),
        corrupted_offer['netuid'],
        corrupted_offer['nonce'],
        corrupted_offer['expire'],
        corrupted_offer['maxTaoAmount'],
        corrupted_offer['maxAlphaPrice'],
        corrupted_offer['dailyInterestRate'],
        bytes.fromhex(corrupted_offer['signature'][2:])  # CORRUPTED signature
    )

    print_info(f"Attempting to cancel with corrupted signature...")
    ...
    try:
        ...
        if "bad signature" in error_str.lower() or "signature" in error_str.lower():
            reverted = True
            print_success("✓ Transaction reverted with expected reason (bad signature)")
        else:
    ...
    print_success("Cancel operation correctly reverted for invalid signature")
    print_success("Verification: signature verification working correctly")
