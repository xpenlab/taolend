#!/usr/bin/env python3
"""
Test Case TC02: Cancel Expired Offer
Objective: Verify cancel(Offer) reverts when offer has expired
Tests: verifyOffer modifier - require(_offer.expire > block.timestamp, "offer expired")

Strategy: Create offer with expired timestamp, attempt to cancel, expect revert
Expected: Transaction reverts with "offer expired" error
"""
...
def main():
    print_section("Test Case TC02: Cancel Expired Offer")
    print(f"{CYAN}Objective:{NC} Verify cancel(Offer) reverts when offer has expired")
    print(f"{CYAN}Strategy:{NC} Use offer with expired timestamp, attempt cancel")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'offer expired'\n")
    ...
    # Find an EXPIRED offer from LENDER1
    offers_dir = Path(__file__).parent.parent / "offers"
    offer_file = None
    offer = None

    print_info("Searching for expired offers from LENDER1...")
    ...
    for file in offer_files:
        ...
        if candidate_offer['lender'].lower() == lender_address.lower():
            # We want an EXPIRED offer (expire <= current_timestamp)
            if candidate_offer['expire'] <= current_timestamp:
                offer_file = file
                offer = candidate_offer
                print_success(f"Found expired offer: {offer_file.name}")
                print_info(f"  Offer ID: {offer['offerId'][:10]}...")
                print_info(f"  Expire: {offer['expire']}")
                print_info(f"  Current: {current_timestamp}")
                print_info(f"  Expired by: {current_timestamp - offer['expire']} seconds")
                break

    if not offer_file:
        print_warning("No expired offer found.")
        print_info("Need to create an expired offer for testing.")
        print_info("Run: python3 scripts/cli.py create-offer --account LENDER1 --max-tao 100 --alpha-price 0.025 --daily-rate 1.0 --netuid 3 --expire-blocks 1")
        print_info("Wait for it to expire (about 12 seconds), then run this test again.")
        sys.exit(1)
    ...
    if current_timestamp < offer['expire']:
        print_error(f"SETUP ERROR: Offer has NOT expired yet!")
        print_error(f"  Current timestamp: {current_timestamp}")
        print_error(f"  Expire timestamp: {offer['expire']}")
        print_error(f"  Wait {offer['expire'] - current_timestamp} more seconds")
        sys.exit(1)

    print_success(f"✓ Offer IS expired:")
    ...
    print_section("Step 4: Execute cancel(Offer) with EXPIRED offer")

    print(f"\n{BOLD}{RED}Expected Result:{NC}")
    print(f"  {RED}Revert:{NC} Transaction reverts (status=0)")
    print(f"  {YELLOW}Reason:{NC} 'offer expired'")
    ...
    print_info(f"Attempting to cancel expired offer {offer['offerId'][:10]}...")
    ...
    try:
        ...
        if "offer expired" in error_str.lower() or "expired" in error_str.lower():
            reverted = True
            revert_reason = "offer expired"
            print_success("✓ Transaction reverted with expected reason")
        else:
    ...
    if tx_receipt:
        ...
        print_info(f"\nGas consumed: {gas_used_wei / 1e18:.9f} TAO")

        actual_diff = lender_evm_after - lender_evm_before
        if abs(actual_diff + gas_used_wei) < 1e9:  # Within 1 Gwei tolerance
            print_success(f"✓ Only gas consumed: {actual_diff / 1e18:.9f} TAO")
        else:
            print_warning(f"Balance change: {actual_diff / 1e18:.9f} TAO")

    # Report results
    print_section("Test Result")

    print_success("✓✓✓ TEST PASSED ✓✓✓")
    print_success("Cancel operation correctly reverted for expired offer")
    print_success("Verification: offer expired check working correctly")
