#!/usr/bin/env python3
"""
Test Case TC04: Loan CLAIMED
Objective: Verify collect fails when loan is already claimed (seized)
...
def main():
    print_section("Test Case TC04: Loan CLAIMED")
    print(f"{CYAN}Objective:{NC} Verify collect fails when loan is already claimed (seized)")
    print(f"{CYAN}Strategy:{NC} Attempt to collect loan in CLAIMED state")
    ...
    if loan_id is None:
        ...
        print_error("Example: python test/test_seize_success.py")
        sys.exit(1)
    ...
    print_warning(f"⚠️  Loan already claimed - collateral seized by lender")
    ...
    print(f"  {CYAN}Reason:{NC} Loan is already in CLAIMED state")
    ...
    print_warning(f"⚠️  Loan state: CLAIMED (lender already seized collateral)")
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Collect correctly rejected CLAIMED loan")
    else:
    ...
    print_success("✓ TC04 PASSED: Collect correctly rejects CLAIMED loan")
    print_info("State protection: Cannot collect claimed loans")
