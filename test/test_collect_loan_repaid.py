#!/usr/bin/env python3
"""
Test Case TC03: Loan REPAID
Objective: Verify collect fails when loan is already repaid
...
def main():
    print_section("Test Case TC03: Loan REPAID")
    print(f"{CYAN}Objective:{NC} Verify collect fails when loan is already repaid")
    print(f"{CYAN}Strategy:{NC} Attempt to collect loan in REPAID state")
    ...
    # Find a loan in REPAID state
    print_info("Finding a loan in REPAID state...")
    ...
    for test_loan_id in range(next_loan_id - 1, max(0, next_loan_id - 100), -1):
        try:
            ...
            if loan_info and loan_info['data']['state'] == STATE_REPAID:
                ...
                print_success(f"Found loan in REPAID state: ID={loan_id}")
                print_info(f"  Lender: {lender_address}")
                print_info(f"  Borrower: {borrower_address}")
                print_info(f"  State: REPAID (2)")
    ...
    if loan_id is None:
        print_error("No loans found in REPAID state")
        print_error("Please run a repay test first to create a loan in REPAID state")
        print_error("Example: python test/test_repay_success.py")
        sys.exit(1)
    ...
    print_info(f"State: {loan_data_before['state']} (REPAID)")
    print_warning(f"⚠️  Loan already repaid - cannot collect")
    ...
    print_warning(f"⚠️  Loan state: REPAID (cannot collect repaid loan)")
    ...
    print_info(f"Loan ID: {loan_id}")
    print_info(f"State: {loan_data_after['state']} (still REPAID)")
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Collect correctly rejected REPAID loan")
    else:
    ...
    print_success("✓ TC03 PASSED: Collect correctly rejects REPAID loan")
    print_info("State protection: Cannot collect repaid loans")
