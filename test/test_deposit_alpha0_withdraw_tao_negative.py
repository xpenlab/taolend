#!/usr/bin/env python3
"""
Test Case 04 (Negative): Deposit netuid=0 ALPHA → Withdraw MORE TAO (Cross-Type, Insufficient Balance)

Test Strategy:
- Deposit 100 TAO via depositAlpha(netuid=0, 100 TAO) (removes from on-chain stake)
- Try to withdraw 150 TAO via withdrawTao(150 TAO) (more than deposited)
...
- Proves: Cross-type withdraw validation works (inverse direction)

This test follows the negative test pattern:
1. Step 0: Verify setup conditions
2. Step 1: Capture balances BEFORE
3. Step 2: Execute deposit ALPHA(netuid=0) transaction (should succeed)
4. Step 3: Capture balances AFTER DEPOSIT
5. Step 4: Execute withdraw TAO with EXCESS amount (should fail)
...
def main():
    print_section("Test Case 04 (Negative): Deposit netuid=0 ALPHA → Withdraw MORE TAO")
    print_info("Objective: Verify inverse cross-type withdraw fails when amount > balance")
    ...
    # Check if user has enough on-chain stake
    if 'staking' in before['balances']['lender1']:
        ...
        if initial_stake_rao < DEPOSIT_AMOUNT_RAO:
            ...
            return
        print_success(f"✓ Sufficient on-chain stake for deposit")
    else:
        print_error("Could not get initial stake from snapshot")
        return

    # Step 2: Execute Deposit ALPHA(netuid=0) (Should Succeed)
    print_section("Step 2: Execute Deposit ALPHA (netuid=0) (Should Succeed)")
    ...
    # Step 4: Execute Withdraw TAO with EXCESS amount (Should Fail)
    ...
    print_success("="*80)
    print_success("✓ TEST PASSED: Deposit netuid=0 ALPHA → Withdraw MORE TAO (Negative Test)")
    print_success("="*80)
    print_success("")
    print_success("All verifications passed:")
    print_success("  ✓ Deposit ALPHA(netuid=0) transaction succeeded")
    print_success("  ✓ Withdraw TAO transaction FAILED as expected")
    ...
    print_success("")
    print_success("Inverse Cross-Type Negative Test Behavior Verified:")
    print_success(f"  Deposited via depositAlpha(netuid=0): {DEPOSIT_AMOUNT_TAO} TAO")
    print_success(f"  Attempted to withdraw via withdrawTao(): {WITHDRAW_AMOUNT_TAO} TAO")
    ...
    print_success("  Inverse cross-type withdrawal validation works correctly")
    print_success("  depositAlpha(netuid=0) and withdrawTao() share same storage")
    print_success("  Both access userAlphaBalance[user][0]")
    print_success("  Balance check applies regardless of deposit/withdrawal method")
    print_success("")
    print_success("Complete Cross-Type Negative Test Matrix:")
    print_success("  TC01-NEG: depositTao()       → withdrawTao(excess)       ✅ REJECTED")
    print_success("  TC02-NEG: depositAlpha(n>0)  → withdrawAlpha(n>0, excess) ✅ REJECTED")
    print_success("  TC03-NEG: depositTao()       → withdrawAlpha(0, excess)   ✅ REJECTED")
    print_success("  TC04-NEG: depositAlpha(0)    → withdrawTao(excess)        ✅ REJECTED")
    print_success("")
