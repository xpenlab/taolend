#!/usr/bin/env python3
"""
Test Case TC18: depositAlpha() - Large Deposit
Objective: Verify depositAlpha works correctly with large deposit amount
Tests: Large deposit flow with 200 ALPHA to netuid=3

Strategy: 8-step testing pattern with BalanceChecker
Expected: Transaction succeeds, balances update correctly

Note: This test requires the user to have sufficient ALPHA staked on the source delegate hotkey
"""
...
def main():
    print_section("Test Case TC18: depositAlpha() - Large Deposit")
    print(f"{CYAN}Objective:{NC} Verify depositAlpha works correctly with large deposit amount")
    print(f"{CYAN}Strategy:{NC} Deposit 200 ALPHA to netuid=3 and verify state changes")
    ...
    deposit_amount_rao = 200 * 10**9  # 200 ALPHA in RAO (large deposit)
    ...
    if all_checks_passed:
        print_success("✓✓✓ TC18 TEST PASSED ✓✓✓")
        print_success("depositAlpha() with large amount executed successfully")
