#!/usr/bin/env python3
"""
Test Case TC15: depositAlpha() - Same Hotkey (if=false)
Objective: Verify depositAlpha works when using DELEGATE_HOTKEY (no move operation)
Tests: if (_delegateHotkey != DELEGATE_HOTKEY) condition evaluates to FALSE

Strategy: 8-step testing pattern with BalanceChecker
Expected: Transaction succeeds, no moveStake call, balances update correctly

Note: This tests the FALSE branch of the if condition at line 1003
"""
...
from const import LENDING_POOL_V2_ADDRESS, DEFAULT_HOTKEY
...
def main():
    print_section("Test Case TC15: depositAlpha() - Same Hotkey (if=false)")
    print(f"{CYAN}Objective:{NC} Verify depositAlpha works when using DELEGATE_HOTKEY")
    print(f"{CYAN}Strategy:{NC} Deposit ALPHA using DELEGATE_HOTKEY (no move operation)")
    print(f"{CYAN}Expected:{NC} Transaction succeeds, if condition = FALSE, no moveStake call\n")
    ...
    # Use DEFAULT_HOTKEY (same as contract's DELEGATE_HOTKEY)
    # This should NOT trigger the moveStake operation
    delegate_hotkey_ss58 = DEFAULT_HOTKEY
    ...
    # Verify hotkeys match
    if delegate_hotkey_bytes != contract_delegate_hotkey:
        print_error(f"SETUP ERROR: Hotkeys don't match!")
        print_error(f"  Test hotkey:     0x{delegate_hotkey_bytes.hex()}")
        print_error(f"  Contract hotkey: 0x{contract_delegate_hotkey.hex()}")
        print_error(f"This test requires using the same hotkey as DELEGATE_HOTKEY")
        sys.exit(1)
    print_success(f"✓ Using DELEGATE_HOTKEY (if condition will be FALSE)")
    ...
    print(f"  {CYAN}Conditional Branch:{NC} if (_delegateHotkey != DELEGATE_HOTKEY) = FALSE")
    print(f"  {CYAN}Behavior:{NC} Only transferStake(), NO moveStake()")
    ...
    print_info(f"Depositing {deposit_amount_rao / 1e9:.2f} ALPHA using DELEGATE_HOTKEY (same as contract)...")
    ...
    if all_checks_passed:
        print_success("✓✓✓ TC15 TEST PASSED ✓✓✓")
        print_success("depositAlpha() with DELEGATE_HOTKEY executed successfully")
        print_success("Conditional branch (if=false) verified: No moveStake() called")
        ...
        print(f"  - Used hotkey: DELEGATE_HOTKEY (same as contract)")
        print(f"  - Operation: transferStake() only (no moveStake)")
        print(f"  - Gas used: {tx_receipt['gasUsed']} units")

        return 0
    else:
        print_error("✗✗✗ TC15 TEST FAILED ✗✗✗")
