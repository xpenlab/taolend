#!/usr/bin/env python3
"""
Test Case TC04: depositTao() - Not Registered
Objective: Verify depositTao fails when user is not registered
...
def main():
    print_section("Test Case TC04: depositTao() - Not Registered")
    print(f"{CYAN}Objective:{NC} Verify depositTao fails when user is not registered")
    ...
    user_evm_balance = w3.eth.get_balance(unregistered_address)
    ...
    # 5. Verify user contract balance unchanged (should be 0)
    user_contract_before = snapshot_before['balances'][unregistered_label]['contract'].get('netuid_0', {})
    user_contract_after = snapshot_after['balances'][unregistered_label]['contract'].get('netuid_0', {})
    ...
    if all_checks_passed:
        print_success("✓✓✓ TC04 TEST PASSED ✓✓✓")
        print_success("depositTao() correctly reverted with 'not registered'")
        print_success("All state validations passed")
        print_success("No unexpected state changes detected")

        print(f"\n{CYAN}Summary:{NC}")
        print(f"  - Unregistered user cannot call depositTao()")
