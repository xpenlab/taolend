#!/usr/bin/env python3
"""
Test Case TC04-02: disableSubnet - Bad Netuid
Objective: Verify disableSubnet fails when netuid is 0
...
def main():
    print_section("Test Case TC04-02: disableSubnet - Bad Netuid")
    print(f"{CYAN}Objective:{NC} Verify disableSubnet fails when netuid is 0")
    print(f"{CYAN}Strategy:{NC} MANAGER attempts to disable netuid 0 (TAO, invalid)")
    ...
    # Test parameters
    test_netuid = 0  # Invalid netuid (TAO is netuid 0, cannot be disabled)

    print_info(f"Manager: {manager_address}")
    print_info(f"Test netuid: {test_netuid} (TAO, invalid for disabling)")
    ...
    print_success(f"✓ Test netuid is 0 (TAO subnet, invalid)")
    ...
    print(f"    - netuid 0 is TAO (cannot be disabled)")
    print(f"    - Only netuid > 0 can be managed")
    print(f"  {CYAN}State Changes:{NC}")
    print(f"    - No state changes except gas deduction\n")

    print_info(f"Attempting to disable netuid {test_netuid} (invalid)...")
    ...
    try:
        ...
        if receipt['status'] == 0:
            ...
            print_error("Should not be able to disable netuid 0 (TAO)")
            sys.exit(1)
    ...
    if all_checks_passed:
        ...
        print_success("Cannot disable netuid 0 (TAO subnet)")
