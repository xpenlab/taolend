#!/usr/bin/env python3
"""
Test Case TC04-04: disableSubnet - High alphaOutPool
Objective: Verify disableSubnet fails when alphaOutPool is above threshold
Tests: _lowAlphaInPool validation check (checks both alphaInPool and alphaOutPool)
...
def main():
    print_section("Test Case TC04-04: disableSubnet - High alphaOutPool")
    print(f"{CYAN}Objective:{NC} Verify disableSubnet fails when alphaOutPool > threshold")
    ...
    test_netuid = 3  # Should have high alphaOutPool
    ...
    # Verify high alphaOutPool condition
    if alpha_out_pool < MIN_POOL_ALPHA_THRESHOLD:
        print_error(f"SETUP ERROR: alphaOutPool ({alpha_out_pool / 1e9:.6f}) is below threshold")
        print_error(f"This test requires alphaOutPool >= {MIN_POOL_ALPHA_THRESHOLD / 1e9:.6f} ALPHA")
        print_warning("Test condition not met - SKIPPING TEST")
        sys.exit(0)

    print_success(f"✓ alphaOutPool ({alpha_out_pool / 1e9:.6f}) >= threshold ({MIN_POOL_ALPHA_THRESHOLD / 1e9:.6f})")
    ...
    print(f"  {CYAN}Reason:{NC} alphaOutPool ({alpha_out_pool / 1e9:.6f}) >= threshold ({MIN_POOL_ALPHA_THRESHOLD / 1e9:.6f})")
    print(f"  {CYAN}State Changes:{NC}")
    print(f"    - No state changes except gas deduction\n")

    print_info(f"Attempting to disable subnet {test_netuid} with high alphaOutPool...")
    ...
    try:
        ...
        if receipt['status'] == 0:
            ...
            print_error("Should not be able to disable subnet with high alphaOutPool")
            sys.exit(1)
    ...
    print_info(f"  alphaInPool: {alpha_in_pool / 1e9:.6f} ALPHA")
    print_info(f"  alphaOutPool: {alpha_out_pool / 1e9:.6f} ALPHA (>= {MIN_POOL_ALPHA_THRESHOLD / 1e9:.6f})")
    print_info(f"  _lowAlphaInPool() should return false (high liquidity)")
    print_success("✓ High alphaOutPool check prevented subnet disable")
    ...
    if all_checks_passed:
        ...
        print_success(f"Cannot disable subnet with alphaOutPool ({alpha_out_pool / 1e9:.6f}) >= threshold")
