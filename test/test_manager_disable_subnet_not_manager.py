#!/usr/bin/env python3
"""
Test Case TC04-01: disableSubnet - Not Manager
Objective: Verify disableSubnet fails when called by non-manager
...
def main():
    print_section("Test Case TC04-01: disableSubnet - Not Manager")
    print(f"{CYAN}Objective:{NC} Verify disableSubnet fails when called by non-manager")
    print(f"{CYAN}Strategy:{NC} Non-manager (LENDER1) attempts to disable a subnet")
    ...
    # Test parameters
    test_netuid = 3  # Use any valid netuid
    ...
    print_info(f"Attempting to disable subnet {test_netuid} as non-manager...")
    ...
    try:
        ...
        if receipt['status'] == 0:
            ...
            print_error("Non-manager should NOT be able to disable subnet")
            sys.exit(1)
    ...
    if all_checks_passed:
        ...
        print_success("Non-manager cannot disable subnet")
