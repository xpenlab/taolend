#!/usr/bin/env python3
"""
Test Case TC06-04: resolveAlpha - Zero Address
Objective: Verify resolveAlpha fails when user address is zero
Tests: require(_user != address(0), "zero address")
...
def main():
    print_section("Test Case TC06-04: resolveAlpha - Zero Address")
    print(f"{CYAN}Objective:{NC} Verify resolveAlpha fails when user address is zero")
    print(f"{CYAN}Strategy:{NC} Manager attempts to resolve ALPHA for address(0)")
    ...
    # Test parameters
    test_user = "0x0000000000000000000000000000000000000000"  # ZERO address (invalid)
    test_netuid = 3
    test_tao_amount = int(10 * 1e9)  # 10 TAO

    print_info(f"Caller: {manager_address} (MANAGER)")
    print_info(f"Test user: {test_user} (ZERO ADDRESS - invalid)")
    ...
    print_info(f"This test will fail on 'zero address' check (before subnet/alpha checks)")
    ...
    # ========================================================================
    print_section("Step 4: Execute resolveAlpha with Zero Address (expect REVERT)")

    print(f"\n{BOLD}{RED}Expected Result:{NC}")
    print(f"  {RED}Failure:{NC} Transaction reverts (status=0)")
    print(f"  {RED}Error:{NC} 'zero address'")
    print(f"  {CYAN}State Changes:{NC}")
    print(f"    - No state changes except gas deduction\n")

    print_info(f"Attempting to resolve ALPHA for address(0)...")
    ...
    try:
        ...
        if receipt['status'] == 0:
            ...
            print_error("resolveAlpha should NOT accept zero address")
            sys.exit(1)

    except Exception as e:
        error_message = str(e)
        if "zero address" in error_message.lower() or "subnet enabled" in error_message.lower():
            print_success(f"✓ Transaction reverted with expected error")
            print_info(f"Error message: {error_message[:200]}")
            print_info("Note: May fail on 'subnet enabled' check before reaching 'zero address' check")
        else:
    ...
    print_success(f"✓ Zero address correctly rejected")
    ...
    if all_checks_passed:
        ...
        print_success("resolveAlpha rejects zero address")
        print_success("User address must be valid (non-zero)")
    else:
