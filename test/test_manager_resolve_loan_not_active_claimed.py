#!/usr/bin/env python3
"""
Test Case TC05-03: resolveLoan - Loan Not Active (CLAIMED)
...
def main():
    print_section("Test Case TC05-03: resolveLoan - Loan Not Active (CLAIMED)")
    ...
    # Use Loan 3 which is in CLAIMED state
    loan_id = 3
    STATE_CLAIMED = 3
    ...
    try:
        ...
        if loan_state != STATE_CLAIMED:
            print_error(f"Loan {loan_id} is not in CLAIMED state (current state={loan_state})")
            print_warning(f"Test condition not met - SKIPPING TEST")
            sys.exit(0)

        print_success(f"✓ Loan {loan_id} is in CLAIMED state (state={loan_state})")
    ...
    print_info(f"Loan {loan_id} State: CLAIMED (state={initial_loan_state})")
    ...
    print(f"  {CYAN}Reason:{NC} Loan is in CLAIMED state (not OPEN or IN_COLLECTION)")
    ...
    print_info(f"Loan {loan_id} State: CLAIMED (state={final_loan_state})")
    ...
    # Check 1: Loan state unchanged (still CLAIMED)
    if final_loan_state == initial_loan_state == STATE_CLAIMED:
        print_success(f"✓ Loan state unchanged: CLAIMED (state={STATE_CLAIMED})")
    else:
    ...
    print_info(f"  Loan state: CLAIMED (state={STATE_CLAIMED})")
    print_info(f"  _requireLoanActive() requires state OPEN(0) or IN_COLLECTION(1)")
    print_info(f"  CLAIMED state should be rejected")
    ...
    if all_checks_passed:
        ...
        print_success("Cannot resolve loan in CLAIMED state")
