#!/usr/bin/env python3
"""
Test Case TC05-04: resolveLoan - Subnet Enabled
Objective: Verify resolveLoan fails when subnet is still enabled
Tests: require(!activeSubnets[netuid], "subnet enabled")

Strategy: 8-step testing pattern with BalanceChecker
Expected: Transaction reverts with "subnet enabled"
"""
...
def main():
    print_section("Test Case TC05-04: resolveLoan - Subnet Enabled")
    print(f"{CYAN}Objective:{NC} Verify resolveLoan fails when subnet is still enabled")
    print(f"{CYAN}Strategy:{NC} Manager attempts to resolve loan on active subnet")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'subnet enabled'\n")
    ...
    # Test parameters
    test_netuid = 2  # Use subnet 2 (typically enabled)
    ...
    if not subnet_active:
        ...
        if tx_receipt['status'] == 1:
            print_success(f"✓ Subnet {test_netuid} enabled")
            subnet_active = True
        else:
            print_error("Failed to enable subnet")
            sys.exit(1)
    else:
        print_success(f"✓ Subnet {test_netuid} is active (enabled)")
    ...
    initial_subnet_status = subnet_active
    ...
    print_info("Not applicable - testing validation check before loan access")

    # ========================================================================
    # Step 4: Execute Test Operation
    # ========================================================================
    print_section("Step 4: Execute Test Operation - Attempt resolveLoan on Active Subnet")

    print_info(f"Calling resolveLoan as MANAGER on active subnet...")
    print_info(f"This should fail with 'subnet enabled' error")
    ...
    try:
        ...
        if tx_receipt['status'] == 0:
            ...
            print_error("Expected: Transaction should revert with 'subnet enabled'")
            sys.exit(1)

    except Exception as e:
        error_msg = str(e)
        if "subnet enabled" in error_msg.lower() or "loan inactive" in error_msg.lower():
            ...
            print_info("This is expected - subnet is enabled, resolveLoan should fail")
    ...
    # Verify subnet status unchanged
    final_subnet_status = contract.functions.activeSubnets(test_netuid).call()
    ...
    if final_subnet_status == initial_subnet_status:
        print_success(f"✓ Subnet status unchanged: {final_subnet_status}")
    else:
        print_error(f"✗ Subnet status changed unexpectedly")
    ...
    print_success("Cannot resolve loan while subnet is active")
    print_success("Subnet must be disabled before manual resolution")
