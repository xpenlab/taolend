#!/usr/bin/env python3
"""
Test Case TC02-01: withdrawProtocolFees - Not Manager
Objective: Verify withdrawProtocolFees fails when called by non-manager
...
def main():
    print_section("Test Case TC02-01: withdrawProtocolFees - Not Manager")
    print(f"{CYAN}Objective:{NC} Verify withdrawProtocolFees fails when called by non-manager")
    print(f"{CYAN}Strategy:{NC} Non-manager (LENDER1) attempts to withdraw protocol fees")
    ...
    # Test parameters
    test_amount = int(10 * 1e9)  # 10 TAO (valid amount)

    print_info(f"Caller: {lender1_address} (LENDER1, non-manager)")
    print_info(f"Manager: {manager_address}")
    print_info(f"Withdrawal amount: {test_amount / 1e9} TAO")
    ...
    # Query protocol fees (for information only, not required for this test)
    ...
    if protocol_fee_accumulated < test_amount:
        print_warning(f"⚠ Protocol fees ({protocol_fee_accumulated / 1e9} TAO) < withdrawal amount ({test_amount / 1e9} TAO)")
        print_info("This test will still fail with 'not manager' before checking fees")
    ...
    print_info(f"Attempting to withdraw {test_amount / 1e9} TAO as non-manager...")
    ...
    try:
        ...
        if receipt['status'] == 0:
            ...
            print_error("Non-manager should NOT be able to withdraw protocol fees")
            sys.exit(1)
    ...
    # Check 2: No balance changes except gas
    ...
    if all_checks_passed:
        ...
        print_success("Non-manager cannot withdraw protocol fees")
