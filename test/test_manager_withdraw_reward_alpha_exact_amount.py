#!/usr/bin/env python3
"""
Test Case TC01-05: withdrawRewardAlpha - Success Exact Amount
Objective: Verify successful withdrawal of all available rewards
Tests: Withdraw all rewards, verify no rewards remain after withdrawal

Strategy: 8-step testing pattern with BalanceChecker
Expected: Transaction succeeds, all rewards withdrawn, no rewards remaining
"""
...
def main():
    print_section("Test Case TC01-05: withdrawRewardAlpha - Success Exact Amount")
    print(f"{CYAN}Objective:{NC} Verify successful withdrawal of all available rewards")
    print(f"{CYAN}Strategy:{NC} MANAGER withdraws ALL available rewards (exact amount)")
    print(f"{CYAN}Expected:{NC} Transaction succeeds, no rewards remain after withdrawal\n")
    ...
    # Set test amount - withdraw ALL available rewards (exact amount)
    test_amount = withdrawable_reward

    print_success(f"✓ Rewards available: {withdrawable_reward / 1e9} ALPHA")
    print_info(f"Will withdraw: {test_amount / 1e9} ALPHA (ALL available rewards)")
    ...
    print(f"    - After withdrawal: totalStake = subnetAlphaBalance (no rewards remaining)\n")

    print_info(f"Withdrawing {test_amount / 1e9} ALPHA from netuid {test_netuid} (ALL rewards)...")
    ...
    # Check 1: On-chain staking decreased by exact amount
    ...
    # Check 3: No remaining rewards (CRITICAL for exact amount test)
    ...
    if remaining_reward == 0:
        print_success(f"✓ No rewards remaining after exact amount withdrawal")
    else:
        print_warning(f"⚠ Warning: {remaining_reward / 1e9} ALPHA rewards still remaining")
        print_info("This may be due to new rewards accumulated during withdrawal")

    # Check 4: totalStake equals subnetAlphaBalance after withdrawal
    if final_total_stake == final_subnet_balance:
        print_success(f"✓ totalStake = subnetAlphaBalance after withdrawal (no excess)")
    else:
        print_warning(f"⚠ totalStake ({final_total_stake / 1e9}) != subnetAlphaBalance ({final_subnet_balance / 1e9})")
        print_info("This may be due to new rewards accumulated during withdrawal")

    # Check 5: Contract state (other fields should be unchanged)
    ...
    if all_checks_passed:
        ...
        print_success(f"Successfully withdrew ALL {test_amount / 1e9} ALPHA in staking rewards")
        print_success(f"On-chain staking decreased by exact amount: {test_amount / 1e9} ALPHA")
        ...
        print_success("✓ Exact amount withdrawal verified successfully")
    else:
