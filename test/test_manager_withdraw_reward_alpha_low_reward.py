#!/usr/bin/env python3
"""
Test Case TC01-03: withdrawRewardAlpha - Low Reward
Objective: Verify withdrawRewardAlpha fails when withdrawable rewards are insufficient
Tests: require(withdrawableReward >= _amount, "low reward")

Strategy: 8-step testing pattern with BalanceChecker
Expected: Transaction reverts with "low reward"
"""
...
def main():
    print_section("Test Case TC01-03: withdrawRewardAlpha - Low Reward")
    print(f"{CYAN}Objective:{NC} Verify withdrawRewardAlpha fails when withdrawable rewards are insufficient")
    print(f"{CYAN}Strategy:{NC} MANAGER attempts to withdraw more than available rewards")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'low reward'\n")
    ...
    # Set test amount to exceed withdrawable reward
    if withdrawable_reward == 0:
        # No rewards, try to withdraw any amount
        test_amount = 1 * 10**9  # 1 ALPHA
        print_info(f"No rewards available, attempting to withdraw {test_amount / 1e9} ALPHA")
    else:
        # Try to withdraw more than available
        test_amount = withdrawable_reward + (100 * 10**9)  # withdrawable + 100 ALPHA
        print_info(f"Attempting to withdraw {test_amount / 1e9} ALPHA (exceeds available by 100 ALPHA)")
    ...
    print(f"  {RED}Revert:{NC} Transaction reverts with 'low reward'")
    print(f"  {CYAN}Reason:{NC} Withdrawable reward ({withdrawable_reward / 1e9} ALPHA) < requested amount ({test_amount / 1e9} ALPHA)")
    ...
    try:
        ...
        if receipt['status'] == 1:
            ...
            print_error("CRITICAL: Able to withdraw more than available rewards!")
            sys.exit(1)
        else:
        ...
        if "low reward" in error_message.lower():
            print_success("✓ Error message contains 'low reward'")
        else:
            print_warning(f"Warning: Error message does not contain 'low reward'")
    ...
    # Check 3: Verify reward calculation
    print_info(f"\nReward Calculation Verification:")
    print_info(f"  Total stake: {total_stake / 1e9} ALPHA")
    print_info(f"  Subnet balance: {subnet_alpha_balance / 1e9} ALPHA")
    print_info(f"  Withdrawable: {withdrawable_reward / 1e9} ALPHA")
    print_info(f"  Requested: {test_amount / 1e9} ALPHA")

    if withdrawable_reward < test_amount:
        print_success(f"✓ Correctly prevented withdrawal (withdrawable < requested)")
    else:
        print_error(f"✗ Withdrawal should have been allowed")
        all_checks_passed = False
    ...
    if all_checks_passed:
        ...
        print_success(f"withdrawRewardAlpha correctly rejects insufficient reward withdrawal")
        print_success(f"Withdrawable: {withdrawable_reward / 1e9} ALPHA, Requested: {test_amount / 1e9} ALPHA")
        print_success("Transaction reverted with 'low reward' error")
