#!/usr/bin/env python3
"""
Test Case TC14: Borrow Less, Insufficient TAO
Objective: Verify refinance fails when borrower wants to borrow less but has insufficient TAO to pay difference
Tests: Borrower balance check - require(userAlphaBalance[msg.sender][0] >= additionalPayment, "low tao")
...
def main():
    print_section("Test Case TC14: Borrow Less, Insufficient TAO")
    print(f"{CYAN}Objective:{NC} Verify refinance fails when borrower wants to borrow less but has insufficient TAO")
    print(f"{CYAN}Strategy:{NC} Refinance with lower loan amount, but borrower's contract TAO is insufficient for difference")
    ...
    blocks_elapsed = current_block - loan_data['startBlock']

    # Get offer details for interest rate
    old_offer_id = loan_data['offerId']
    old_offer = contract.functions.loanOffers(old_offer_id).call()
    day_rate = old_offer[4]  # dayRate

    # Calculate interest: loanAmount * dayRate * blocksElapsed / RATE_BASE / BLOCKS_PER_DAY
    RATE_BASE = 10_000_000_000
    BLOCKS_PER_DAY = 7200  # 12s per block

    interest = (loan_amount * day_rate * blocks_elapsed) // (RATE_BASE * BLOCKS_PER_DAY)
    repay_amount = loan_amount + interest

    print_info(f"Loan amount: {loan_amount / 1e9:.2f} TAO")
    print_info(f"Blocks elapsed: {blocks_elapsed}")
    print_info(f"Interest accrued: {interest / 1e9:.4f} TAO")
    print_info(f"Repay amount (with interest): {repay_amount / 1e9:.4f} TAO")

    # Set new loan amount to be LESS than repay amount (borrow less)
    new_loan_amount = int(repay_amount * 0.6)  # Borrow 60% of repay amount
    additional_payment_needed = repay_amount - new_loan_amount

    print_info(f"New loan amount: {new_loan_amount / 1e9:.2f} TAO (borrowing less)")
    print_info(f"Additional payment needed: {additional_payment_needed / 1e9:.4f} TAO")

    # Check borrower's current contract TAO balance
    borrower_tao_balance = contract.functions.userAlphaBalance(borrower_address, 0).call()
    print_info(f"Borrower current TAO balance: {borrower_tao_balance / 1e9:.4f} TAO")

    # Withdraw TAO to make balance insufficient
    if borrower_tao_balance >= additional_payment_needed:
        # Leave some balance but not enough
        target_balance = int(additional_payment_needed * 0.5)  # 50% of what's needed
        withdraw_amount = borrower_tao_balance - target_balance

        print_warning(f"Withdrawing {withdraw_amount / 1e9:.4f} TAO to make balance insufficient...")

        try:
            tx_data = contract.functions.withdrawTao(withdraw_amount).build_transaction({
            ...
            signed_tx = w3.eth.account.sign_transaction(tx_data, borrower_private_key)
            tx_hash = w3.eth.send_raw_transaction(signed_tx.raw_transaction)
            tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=120)

            if tx_receipt['status'] == 1:
                borrower_tao_balance = contract.functions.userAlphaBalance(borrower_address, 0).call()
                print_success(f"✓ Withdrew TAO, new balance: {borrower_tao_balance / 1e9:.4f} TAO")
            else:
    ...
    print_warning(f"Borrower TAO balance ({borrower_tao_balance / 1e9:.4f} TAO) < Required ({additional_payment_needed / 1e9:.4f} TAO)")
    ...
    print(f"  {CYAN}Reason:{NC} Borrower TAO balance ({borrower_tao_balance / 1e9:.4f} TAO) < Additional payment ({additional_payment_needed / 1e9:.4f} TAO)")
    print(f"  {CYAN}Scenario:{NC} Borrow less ({new_loan_amount / 1e9:.2f} TAO < {repay_amount / 1e9:.4f} TAO repay)")
    ...
    print_info(f"Repay amount: {repay_amount / 1e9:.4f} TAO")
    print_info(f"New loan amount: {new_loan_amount / 1e9:.2f} TAO (LESS)")
    print_warning(f"Borrower must pay: {additional_payment_needed / 1e9:.4f} TAO")
    print_warning(f"Borrower has: {borrower_tao_balance / 1e9:.4f} TAO (INSUFFICIENT)")
    ...
    try:
        ...
        error_msg = str(e)
        if 'low tao' in error_msg:
            print_success(f"✓ Gas estimation failed (expected): 'low tao'")
        else:
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Refinance correctly rejected insufficient borrower TAO balance")
    else:
    ...
    if loan_term_before['loanDataId'] == loan_term_after['loanDataId']:
        ...
        print_error(f"✗ Loan data ID changed")
    ...
    print_success("✓ TC14 PASSED: Refinance correctly rejects insufficient borrower TAO for 'borrow less' scenario")
    print_info("Validation tested: Borrower balance check when borrowing less")
    print_info("Error message: 'low tao'")
    print_info(f"Repay amount: {repay_amount / 1e9:.4f} TAO")
    print_info(f"New loan amount: {new_loan_amount / 1e9:.2f} TAO")
    print_info(f"Additional payment needed: {additional_payment_needed / 1e9:.4f} TAO")
    print_info(f"Borrower TAO balance: {borrower_tao_balance / 1e9:.4f} TAO (INSUFFICIENT)")
