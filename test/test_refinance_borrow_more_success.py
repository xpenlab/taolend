#!/usr/bin/env python3
"""
Test Case TC17: Borrow More, Success
Objective: Verify successful refinance when borrower borrows more than repay amount
Tests: Contract unstakes TAO and transfers difference to borrower

Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction succeeds, borrower receives difference, new loan created
"""
...
def main():
    """Test successful refinance when borrowing more"""

    print_section("Test Case TC17: Borrow More, Success")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify successful refinance when borrowing more than repay amount")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} BORROWER refinances with higher amount, receives difference as TAO")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction succeeds, borrower receives difference, new loan created")
    ...
    if current_lender.lower() == addresses['LENDER1']['evmAddress'].lower():
        ...
        new_offer_file = "offers/15768b52bdb73e8b7a9523e677bbcfa312d8fd7b907a3c5630a58f5b41658c10.json"
    ...
    # Set new loan amount to be 120% of repay amount (borrow significantly more)
    new_loan_amount = (repay_amount * 120) // 100  # 120% of repay amount
    ...
    print_info(f"  New Loan Amount: {new_loan_amount / 1e9:.9f} TAO (120% of repay)")
    print_info(f"  Additional Borrow: {additional_borrow / 1e9:.9f} TAO")
    print_info(f"  Scenario: Borrow more, receive difference")
    ...
    # Verify contract has enough TAO to provide additional borrow
    contract_tao_balance = contract.functions.subnetAlphaBalance(0).call()
    print_info(f"Contract TAO Balance: {contract_tao_balance / 1e9:.2f} TAO")
    if contract_tao_balance < additional_borrow:
        print_error(f"✗ Contract has insufficient TAO ({contract_tao_balance / 1e9:.2f} < {additional_borrow / 1e9:.2f})")
        return 1
    print_success(f"✓ Contract has sufficient TAO for additional borrow")
    ...
    print(f"    - BORROWER EVM TAO: +{additional_borrow / 1e9:.9f} TAO (receives difference)")
    print(f"    - BORROWER contract TAO[0]: unchanged")
    ...
    print(f"    - Contract TAO[0]: -{additional_borrow / 1e9:.9f} TAO (unstaked)")
    ...
    print_info(f"Additional Borrow: {additional_borrow / 1e9:.9f} TAO (to be received)")
    ...
    actual_additional_borrow = new_loan_amount - actual_repay_amount
    ...
    print_info(f"  Additional Borrow: {actual_additional_borrow / 1e9:.9f} TAO")
    ...
    print_info("\nExpected changes:")
    print_info(f"  - BORROWER EVM TAO: +{actual_additional_borrow / 1e9:.9f} TAO (received as transfer) minus gas")
    print_info(f"  - BORROWER contract TAO[0]: unchanged")
    ...
    print_info(f"  - Contract TAO[0]: -{actual_additional_borrow / 1e9:.9f} TAO (unstaked)")
    ...
    print_success("TC17: Borrow More, Success")
    ...
    print(f"  - New loan amount: {loan_data_after['loanAmount'] / 1e9:.9f} TAO (120% of repay)")
    print(f"  - BORROWER received: {actual_additional_borrow / 1e9:.9f} TAO as EVM transfer")
    print(f"  - Contract unstaked: {actual_additional_borrow / 1e9:.9f} TAO from netuid 0")
