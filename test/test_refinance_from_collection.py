#!/usr/bin/env python3
"""
Test Case TC19: Refinance from IN_COLLECTION
Objective: Verify successful refinance when loan is in IN_COLLECTION state
Tests: Borrower can refinance from IN_COLLECTION state (rescue refinance)

Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction succeeds, loan refinanced from IN_COLLECTION state
"""
...
def main():
    """Test successful refinance from IN_COLLECTION state"""

    print_section("Test Case TC19: Refinance from IN_COLLECTION")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify successful refinance when loan is in IN_COLLECTION state")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} BORROWER refinances loan that is IN_COLLECTION (rescue refinance)")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction succeeds, loan refinanced from IN_COLLECTION state")
    ...
    # Verify loan is IN_COLLECTION
    if loan_data['state'] != STATE_IN_COLLECTION:
        print_error(f"Loan {loan_id} is not IN_COLLECTION (current: {STATE_NAMES.get(loan_data['state'], 'UNKNOWN')})")
        print_error("This test requires loan in IN_COLLECTION state")
        print_error("Use 'python scripts/cli.py collect --loan-id 11 --account LENDER1' to set state")
        return 1
    ...
    print_success("TC19: Refinance from IN_COLLECTION")
    print_success("Refinance completed successfully from IN_COLLECTION state")
