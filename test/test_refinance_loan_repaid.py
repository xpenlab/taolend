#!/usr/bin/env python3
"""
Test Case TC02: Loan Already Repaid
Objective: Verify refinance fails when loan is in REPAID state  
...
def main():
    print_section("Test Case TC02: Loan Already Repaid")
    print(f"{CYAN}Objective:{NC} Verify refinance fails when loan is in REPAID state")
    print(f"{CYAN}Strategy:{NC} Attempt to refinance a loan that has been repaid")
    ...
    # Find a REPAID loan for testing
    print_info("Finding a REPAID loan for testing...")
    ...
    for test_loan_id in range(next_loan_id - 1, max(0, next_loan_id - 20), -1):
        try:
            ...
            if loan_info and loan_info['data']['state'] == STATE_REPAID:
                ...
                if loan_info['term']['borrower'].lower() == borrower_address.lower():
                    ...
                    print_success(f"Found REPAID loan: ID={loan_id}, netuid={loan_netuid}")
    ...
    if loan_id is None:
        print_error("No REPAID loans found for BORROWER1. Please repay a loan first.")
        sys.exit(1)
    ...
    print_warning(f"State: {loan_data_before['state']} (REPAID)")
    ...
    print(f"  {CYAN}Reason:{NC} Loan is in REPAID state (_requireLoanActive)")
    ...
    print_warning(f"Loan State: REPAID (inactive)")
    ...
    print_info(f"Loan ID: {loan_id}")
    print_info(f"State: {loan_data_after['state']} (REPAID)")
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Refinance correctly rejected REPAID loan")
    else:
    ...
    if loan_data_before['state'] == loan_data_after['state']:
        print_success("✓ Loan state unchanged (REPAID)")
    else:
    ...
    print_success("✓ TC02 PASSED: Refinance correctly rejects REPAID loans")
