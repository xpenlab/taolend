#!/usr/bin/env python3
"""
Test Case TC09: Loan Too Small
Objective: Verify refinance fails when new loan amount is below minimum
Tests: Minimum amount check - require(_newLoanAmount >= MIN_LOAN_AMOUNT, "loan too small")
...
def main():
    print_section("Test Case TC09: Loan Too Small")
    print(f"{CYAN}Objective:{NC} Verify refinance fails when new loan amount is below minimum")
    print(f"{CYAN}Strategy:{NC} Attempt refinance with new loan amount < MIN_LOAN_AMOUNT (0.1 TAO)")
    ...
    # Create offer for NEW_LENDER with same netuid
    ...
    # Calculate refinance amount - TOO SMALL (below MIN_LOAN_AMOUNT)
    MIN_LOAN_AMOUNT = int(1e8)  # 0.1 TAO
    new_loan_amount = int(5e7)  # 0.05 TAO (half of minimum)
    print_warning(f"New loan amount: {new_loan_amount / 1e9:.4f} TAO (BELOW MINIMUM {MIN_LOAN_AMOUNT / 1e9:.1f} TAO)")
    ...
    print(f"  {CYAN}Reason:{NC} New loan amount ({new_loan_amount / 1e9:.4f} TAO) < MIN_LOAN_AMOUNT ({MIN_LOAN_AMOUNT / 1e9:.1f} TAO)")
    ...
    print_warning(f"New Loan Amount: {new_loan_amount / 1e9:.4f} TAO (TOO SMALL)")
    ...
    try:
        ...
        error_msg = str(e)
        if 'loan too small' in error_msg:
            print_success(f"✓ Gas estimation failed (expected): 'loan too small'")
        else:
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Refinance correctly rejected loan amount below minimum")
    else:
    ...
    print_success("✓ TC09 PASSED: Refinance correctly rejects loan amount below minimum")
    print_info("Validation tested: Minimum amount check (require _newLoanAmount >= MIN_LOAN_AMOUNT)")
    print_info("Error message: 'loan too small'")
    print_info(f"MIN_LOAN_AMOUNT: {MIN_LOAN_AMOUNT / 1e9:.1f} TAO")
    print_info(f"Attempted amount: {new_loan_amount / 1e9:.4f} TAO")
