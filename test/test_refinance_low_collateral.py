#!/usr/bin/env python3
"""
Test Case TC13: Low Collateral
Objective: Verify refinance fails when collateral value is insufficient to cover loan amount
Tests: Collateral value check - require(alphaAmount * maxAlphaPrice >= taoAmount * PRICE_BASE, "low collateral")
...
def main():
    print_section("Test Case TC13: Low Collateral")
    print(f"{CYAN}Objective:{NC} Verify refinance fails when collateral value is insufficient to cover loan amount")
    print(f"{CYAN}Strategy:{NC} Create offer with very low maxAlphaPrice, making collateral value insufficient")
    ...
    # For collateral to be sufficient: collateralAmount * maxAlphaPrice >= loanAmount * PRICE_BASE
    # So: maxAlphaPrice >= loanAmount * PRICE_BASE / collateralAmount
    PRICE_BASE = int(1e9)
    required_min_price = (loan_amount * PRICE_BASE) // loan_collateral
    print_info(f"Required minimum ALPHA price: {required_min_price / 1e9:.4f} TAO/ALPHA")
    print_info(f"  (for {loan_collateral / 1e9:.2f} ALPHA to cover {loan_amount / 1e9:.2f} TAO)")

    # Set maxAlphaPrice to be LOWER than required (e.g., 50% of required)
    low_max_alpha_price = int(required_min_price * 0.5)
    print_warning(f"Setting maxAlphaPrice to {low_max_alpha_price / 1e9:.4f} TAO/ALPHA (50% of required - TOO LOW)")

    # Verify this will trigger "low collateral"
    collateral_value = loan_collateral * low_max_alpha_price
    required_value = loan_amount * PRICE_BASE
    print_info(f"Collateral value at offer price: {collateral_value / 1e18:.2f} TAO")
    print_info(f"Required value: {required_value / 1e18:.2f} TAO")
    print_warning(f"Deficit: {(required_value - collateral_value) / 1e18:.2f} TAO (INSUFFICIENT)")

    # Create offer for NEW_LENDER with LOW maxAlphaPrice
    print_info("Creating offer for NEW_LENDER with LOW maxAlphaPrice...")
    ...
    new_offer = create_offer(
        ...
        max_alpha_price=low_max_alpha_price,  # LOW price!
        daily_interest_rate=5_000_000,
        expire_seconds=86400
    )
    print_success(f"Created offer: offerId={new_offer['offerId']}, netuid={new_offer['netuid']}")
    print_warning(f"Offer maxAlphaPrice: {new_offer['maxAlphaPrice'] / 1e9:.4f} TAO/ALPHA (TOO LOW FOR COLLATERAL)")
    ...
    print(f"  {CYAN}Reason:{NC} Collateral value ({collateral_value / 1e18:.2f} TAO) < Required value ({required_value / 1e18:.2f} TAO)")
    print(f"  {CYAN}Formula:{NC} {loan_collateral / 1e9:.2f} ALPHA × {low_max_alpha_price / 1e9:.4f} TAO/ALPHA < {loan_amount / 1e9:.2f} TAO × 1e9")
    ...
    print_warning(f"Offer maxAlphaPrice: {low_max_alpha_price / 1e9:.4f} TAO/ALPHA (TOO LOW)")
    print_info(f"Collateral: {loan_collateral / 1e9:.2f} ALPHA")
    print_info(f"Collateral value: {collateral_value / 1e18:.2f} TAO (INSUFFICIENT)")
    ...
    try:
        ...
        error_msg = str(e)
        if 'low collateral' in error_msg:
            print_success(f"✓ Gas estimation failed (expected): 'low collateral'")
        else:
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Refinance correctly rejected insufficient collateral value")
    else:
    ...
    print_success("✓ TC13 PASSED: Refinance correctly rejects insufficient collateral value")
    print_info("Validation tested: Collateral value check (require alphaAmount * maxAlphaPrice >= taoAmount * PRICE_BASE)")
    print_info("Error message: 'low collateral'")
    print_info(f"Collateral: {loan_collateral / 1e9:.2f} ALPHA")
    print_info(f"Offer price: {low_max_alpha_price / 1e9:.4f} TAO/ALPHA")
    print_info(f"Collateral value: {collateral_value / 1e18:.2f} TAO")
    print_info(f"Required value: {required_value / 1e18:.2f} TAO")
    print_info(f"Deficit: {(required_value - collateral_value) / 1e18:.2f} TAO")
