#!/usr/bin/env python3
"""
Test Case TC21: Refinance to Lower Rate
Objective: Verify successful refinance to better offer with lower daily interest rate
Tests: Borrower refinances to offer with 0.5% rate (down from 1.0%), borrow less scenario

Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction succeeds, new loan has lower rate, borrower benefits from better terms
"""
...
def main():
    """Test successful refinance to lower interest rate offer"""

    print_section("Test Case TC21: Refinance to Lower Rate")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify successful refinance to better offer with lower daily rate")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} BORROWER refinances from 1.0% rate to 0.5% rate offer")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction succeeds, new loan has 0.5% rate instead of 1.0%")
    ...
    current_daily_rate = offer_data['dailyInterestRate']
    ...
    print_info(f"  Current Daily Rate: {current_daily_rate / 1e9 * 100:.4f}%")
    ...
    # Load new lower rate offer (0.5% daily rate)
    new_offer_file = "offers/e52708359c715fbf0658810fe27921058b8766e04d227bafb14d72f734a9215a.json"

    print_info(f"\nLoading new lower rate offer...")
    ...
    new_daily_rate = new_offer['dailyInterestRate']
    ...
    print_info(f"  Daily Rate: {new_daily_rate / 1e9 * 100:.4f}% (down from {current_daily_rate / 1e9 * 100:.4f}%)")
    print_info(f"  Max Alpha Price: {new_offer['maxAlphaPrice'] / 1e9:.9f} TAO/ALPHA")

    # Verify this is a rate improvement
    if new_daily_rate >= current_daily_rate:
        print_error(f"⚠ Test setup error: New rate ({new_daily_rate / 1e9 * 100:.4f}%) >= current rate ({current_daily_rate / 1e9 * 100:.4f}%)")
        print_error("This test requires a lower rate offer")
        return 1

    rate_improvement = ((current_daily_rate - new_daily_rate) / current_daily_rate) * 100
    print_success(f"✓ Rate improvement: {rate_improvement:.1f}% lower")
    ...
    is_registered_new = contract.functions.registeredUser(new_offer_lender).call()
    if not is_registered_new:
        print_error(f"NEW_LENDER must be registered for this test")
        return 1
    print_success(f"✓ NEW_LENDER registered: {new_offer_lender}")

    # Set new loan amount to be 80% of repay amount (borrow less scenario)
    new_loan_amount = (repay_amount * 80) // 100
    ...
    print_info(f"  Scenario: Borrow less, pay difference, get better rate")

    # Verify NEW_LENDER has enough TAO
    new_lender_balance = contract.functions.userAlphaBalance(new_offer_lender, 0).call()
    ...
    snapshot_accounts = [
        ...
        {'label': 'NEW_LENDER', 'address': new_offer_lender},
    ...
    new_lender_lend_balance_before = contract.functions.userLendBalance(new_offer_lender, new_offer_id).call()
    ...
    print_info(f"  Daily Rate: {offer_before['dailyInterestRate'] / 1e9 * 100:.4f}%")
    ...
    print(f"    - Daily rate: {current_daily_rate / 1e9 * 100:.4f}% → {new_daily_rate / 1e9 * 100:.4f}%")
    ...
    print_info(f"Payment Required: {payment_required / 1e9:.9f} TAO")
    print_info(f"Rate Change: {current_daily_rate / 1e9 * 100:.4f}% → {new_daily_rate / 1e9 * 100:.4f}%")
    ...
    new_lender_lend_balance_after = contract.functions.userLendBalance(new_offer_lender, new_offer_id).call()
    ...
    print_info(f"  Daily Rate: {offer_after['dailyInterestRate'] / 1e9 * 100:.4f}% (was: {offer_before['dailyInterestRate'] / 1e9 * 100:.4f}%)")
    ...
    # Verify daily rate changed to lower rate
    if offer_after['dailyInterestRate'] == new_daily_rate:
        print_success(f"✓ Daily rate changed to {new_daily_rate / 1e9 * 100:.4f}%")
    else:
        print_error(f"❌ Daily rate not changed correctly")
        return 1

    # Verify rate improvement
    if offer_after['dailyInterestRate'] < offer_before['dailyInterestRate']:
        rate_improvement = ((offer_before['dailyInterestRate'] - offer_after['dailyInterestRate']) / offer_before['dailyInterestRate']) * 100
        print_success(f"✓ Rate improved by {rate_improvement:.1f}%: {offer_before['dailyInterestRate'] / 1e9 * 100:.4f}% → {offer_after['dailyInterestRate'] / 1e9 * 100:.4f}%")
    else:
        print_error(f"❌ Rate not improved")
        return 1
    ...
    print_success("TC21: Refinance to Lower Rate")
    ...
    print_success("Rate improvement verified")
    ...
    print(f"  - Rate improvement: {offer_before['dailyInterestRate'] / 1e9 * 100:.4f}% → {offer_after['dailyInterestRate'] / 1e9 * 100:.4f}% ({rate_improvement:.1f}% lower)")
