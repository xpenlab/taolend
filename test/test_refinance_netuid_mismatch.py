#!/usr/bin/env python3
"""
Test Case TC08: Netuid Mismatch
Objective: Verify refinance fails when offer netuid doesn't match loan netuid
Tests: Netuid match - require(_offer.netuid == netuid, "netuid mismatch")

Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction reverts with "netuid mismatch"
"""
...
def main():
    print_section("Test Case TC08: Netuid Mismatch")
    print(f"{CYAN}Objective:{NC} Verify refinance fails when offer netuid doesn't match loan netuid")
    print(f"{CYAN}Strategy:{NC} Attempt refinance with offer for different netuid")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'netuid mismatch'\n")
    ...
    # Create offer for DIFFERENT netuid
    print_info("Creating offer for NEW_LENDER with DIFFERENT netuid...")
    ...
    # Choose a different netuid (if loan is netuid 3, use netuid 2)
    different_netuid = 2 if loan_netuid == 3 else 3
    print_warning(f"Loan netuid: {loan_netuid}, Offer netuid: {different_netuid} (MISMATCH)")

    # Query alpha price for the different netuid
    try:
        alpha_price = contract.functions.getAlphaPrice(different_netuid).call()
    ...
    new_offer = create_offer(
        ...
        netuid=different_netuid,  # Different netuid!
    ...
    checker = BalanceChecker(w3, contract, test_netuids=[0, loan_netuid, different_netuid])
    addresses_list = [
    ...
    print(f"  {RED}✗ Revert:{NC} 'netuid mismatch'")
    print(f"  {CYAN}Reason:{NC} Offer netuid ({different_netuid}) != Loan netuid ({loan_netuid})")
    ...
    print_warning(f"Offer Netuid: {different_netuid} (MISMATCH)")
    ...
    try:
        ...
        error_msg = str(e)
        if 'netuid mismatch' in error_msg:
            print_success(f"✓ Gas estimation failed (expected): 'netuid mismatch'")
        else:
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Refinance correctly rejected netuid mismatch")
    else:
    ...
    if loan_term_before['netuid'] == loan_term_after['netuid']:
        print_success(f"✓ Loan netuid unchanged ({loan_netuid})")
    else:
    ...
    print_success("✓ TC08 PASSED: Refinance correctly rejects netuid mismatch")
    print_info("Validation tested: Netuid match (require _offer.netuid == netuid)")
    print_info("Error message: 'netuid mismatch'")
    print_info(f"Loan netuid: {loan_netuid}, Offer netuid: {different_netuid}")
