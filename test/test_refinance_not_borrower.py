#!/usr/bin/env python3
"""
Test Case TC07: Not Borrower
Objective: Verify refinance fails when initiator is not the borrower
Tests: Borrower check - require(loanTerm.borrower == msg.sender, "not borrower")

Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction reverts with "not borrower"
"""
...
def main():
    print_section("Test Case TC07: Not Borrower")
    print(f"{CYAN}Objective:{NC} Verify refinance fails when initiator is not the borrower")
    print(f"{CYAN}Strategy:{NC} Attempt refinance with lender (not borrower)")
    print(f"{CYAN}Expected:{NC} Transaction reverts with 'not borrower'\n")
    ...
    lender_address = addresses['LENDER1']['evmAddress']  # Will attempt refinance
    ...
    # Verify LENDER1 is registered
    lender_registered = contract.functions.registeredUser(lender_address).call()
    if not lender_registered:
        print_error(f"LENDER1 must be registered for this test")
        sys.exit(1)
    print_success(f"LENDER1 is registered")
    ...
    # Find an active loan where BORROWER1 borrowed from LENDER1
    print_info("Finding active loan from LENDER1 to BORROWER1...")
    ...
    for test_loan_id in range(next_loan_id - 1, max(0, next_loan_id - 20), -1):
        try:
            ...
            if loan_info and loan_info['data']['state'] in [STATE_OPEN, STATE_IN_COLLECTION]:
                # Check if BORROWER1 borrowed from LENDER1
                if (loan_info['term']['borrower'].lower() == borrower_address.lower() and
                    loan_info['offer']['lender'].lower() == lender_address.lower()):
                    ...
                    print_info(f"  Lender: {loan_info['offer']['lender']}")
    ...
    if loan_id is None:
        print_error("No active loans found from LENDER1 to BORROWER1.")
        sys.exit(1)
    ...
    addresses_list = [
        ...
        {"address": lender_address, "label": "LENDER1 (trying refinance)"},
    ...
    print(f"  {RED}✗ Revert:{NC} 'not borrower'")
    print(f"  {CYAN}Reason:{NC} Initiator is lender, not borrower (borrower check)")
    ...
    print_warning(f"Initiator: {lender_address} (LENDER, not borrower)")
    print_info(f"Actual Borrower: {borrower_address}")
    ...
    try:
        ...
        error_msg = str(e)
        if 'not borrower' in error_msg:
            print_success(f"✓ Gas estimation failed (expected): 'not borrower'")
        else:
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Refinance correctly rejected non-borrower")
    else:
    ...
    print_success("✓ TC07 PASSED: Refinance correctly rejects non-borrower")
    print_info("Validation tested: Borrower check (require borrower == msg.sender)")
    print_info("Error message: 'not borrower'")
