...
Objective: Verify refinance fails when initiator (borrower) is not registered
...
def main():
    ...
    print(f"{CYAN}Objective:{NC} Verify refinance fails when initiator (borrower) is not registered")
    print(f"{CYAN}Strategy:{NC} Attempt refinance with unregistered borrower")
    ...
    # Find an unregistered account to use as borrower
    print_info("Finding unregistered account to use as borrower...")
    unregistered_borrower_address = None
    unregistered_borrower_private_key = None
    ...
    for account_name in candidate_accounts:
        if account_name in addresses:
            ...
            if not is_registered:
                unregistered_borrower_address = candidate_address
                unregistered_borrower_private_key = os.environ.get(f"{account_name}_PRIVATE_KEY")
                unregistered_label = account_name
                print_success(f"Found unregistered account: {account_name} ({candidate_address})")
                break

    if not unregistered_borrower_address:
        print_error("No unregistered account found. Please ensure at least one account is not registered.")
        sys.exit(1)

    if not unregistered_borrower_private_key:
        print_error(f"Private key not found for {unregistered_label}. Set {unregistered_label}_PRIVATE_KEY in .env")
        sys.exit(1)
    ...
    # Find an existing active loan (we need a loanId for the test)
    print_info("Finding an active loan for testing...")
    # Query next loan ID to find recent loans
    ...
    for test_loan_id in range(next_loan_id - 1, max(0, next_loan_id - 20), -1):
        try:
            ...
            if loan_info and loan_info['data']['state'] in [STATE_OPEN, STATE_IN_COLLECTION]:
                ...
                print_success(f"Found active loan: ID={loan_id}, state={loan_info['data']['state']}, netuid={loan_netuid}")
    ...
    if loan_id is None:
        print_error("No active loans found. Please create a loan first.")
        sys.exit(1)
    ...
    # Try to load existing offer, or create new one
    ...
    if not new_offer:
        ...
        safe_max_alpha_price = int(alpha_price * 0.85)  # 85% of current price (below 90% threshold)
        ...
        new_offer = create_offer(
            ...
            netuid=loan_netuid,
            max_tao_amount=int(200e9),  # 200 TAO
    ...
    # Note: This test will revert, so we don't need exact amount, just reasonable balance
    required_tao = int(20e9)  # ~20 TAO needed (similar to current loan)
    if new_lender_tao_balance < required_tao:
        ...
        print_error("Please deposit more TAO for NEW_LENDER")
        sys.exit(1)
    print_success(f"NEW_LENDER has sufficient TAO: {new_lender_tao_balance / 1e9:.2f} TAO")

    # Calculate refinance amount (same as current loan amount for simplicity)
    ...
    addresses_list = [
        {"address": unregistered_borrower_address, "label": f"BORROWER ({unregistered_label})"},
    ...
    print(f"  {CYAN}Reason:{NC} Borrower not registered (onlyRegistered modifier)")
    print(f"  {CYAN}State Changes:{NC}")
    print(f"    - No state changes (transaction reverts)")
    print(f"    - Only gas deducted from unregistered borrower's EVM TAO")

    print_info(f"\nAttempting to refinance loan {loan_id}...")
    print_warning(f"Initiator: {unregistered_borrower_address} (UNREGISTERED)")
    ...
    nonce = w3.eth.get_transaction_count(unregistered_borrower_address)
    ...
    try:
        gas_estimate = contract.functions.refinance(
            ...
            'from': unregistered_borrower_address,
            'nonce': nonce
        })
        ...
        if 'not registered' in error_msg:
            print_success(f"✓ Gas estimation failed (expected): 'not registered'")
        else:
    ...
    signed_tx = w3.eth.account.sign_transaction(tx, unregistered_borrower_private_key)
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Refinance correctly rejected unregistered borrower")
    else:
    ...
    print_success("✓ TC01 PASSED: Refinance correctly rejects unregistered borrower")
    print_info("Modifier tested: onlyRegistered")
    print_info("Error message: 'not registered'")
