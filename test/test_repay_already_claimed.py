#!/usr/bin/env python3
"""
Test Case TC04: Loan Already Claimed
Objective: Verify repay fails when loan is already in CLAIMED state
Tests: _requireLoanActive - state check (loan.state == CLAIMED)
Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction reverts with "loan inactive"

Note: CLAIMED state occurs when:
  1. Loan enters IN_COLLECTION (lender calls collect())
  2. MIN_LOAN_DURATION passes
  3. Lender calls seize() to claim collateral
"""
...
from common import (
    ...
    STATE_OPEN, STATE_IN_COLLECTION, STATE_REPAID, STATE_CLAIMED
)
...
def main():
    print_section("Test Case TC04: Loan Already Claimed")
    print(f"{CYAN}Objective:{NC} Verify repay fails when loan is already in CLAIMED state")
    print(f"{CYAN}Strategy:{NC} Find a CLAIMED loan (collateral seized by lender), then attempt to repay")
    ...
    for loan_id in range(next_loan_id):
        try:
            ...
            # Check if loan is in CLAIMED state
            if loan_data['state'] == STATE_CLAIMED:
                claimed_loan_id = loan_id
                print_success(f"✓ Found CLAIMED loan: Loan ID {loan_id}")
                ...
                print_info(f"  Seized by: {loan_data['initiator']}")
                break
        except Exception as e:
            continue

    if claimed_loan_id is None:
        print_warning("No CLAIMED loan found in system")
        print_info("\nTo create a CLAIMED loan:")
        print_info("  1. Create a loan (borrower borrows from lender)")
        print_info("  2. Lender calls collect() after MIN_LOAN_DURATION")
        print_info("  3. Wait MIN_LOAN_DURATION blocks")
        print_info("  4. Lender calls seize() to claim collateral")
        print_info("\nThis test requires a loan in CLAIMED state")
        print_error("SETUP ERROR: No CLAIMED loan available for testing")
        sys.exit(1)

    test_loan_id = claimed_loan_id
    ...
    # Verify loan state is CLAIMED
    if loan_data_initial['state'] != STATE_CLAIMED:
        print_error(f"SETUP ERROR: Loan {test_loan_id} is not in CLAIMED state")
        sys.exit(1)
    print_success(f"✓ Confirmed: Loan {test_loan_id} is in CLAIMED state")
    ...
    print_info(f"  Seized by: {loan_data_before['initiator']}")

    # Verify state is CLAIMED
    if loan_data_before['state'] != STATE_CLAIMED:
        print_error(f"ERROR: Loan state is not CLAIMED!")
        sys.exit(1)
    print_success(f"✓ Loan state confirmed: CLAIMED")
    ...
    print(f"  {CYAN}Reason:{NC} Loan is already in CLAIMED state (collateral seized)")
    print(f"  {CYAN}Validation:{NC} _requireLoanActive checks loan.state == CLAIMED")
    print(f"  {CYAN}Business Logic:{NC} Once collateral is claimed, loan cannot be repaid")
    ...
    print_info(f"Attempting to repay claimed loan {test_loan_id}...")
    print_info(f"Repayer: {repayer_address} (BORROWER1)")
    print_warning(f"Note: Collateral has already been seized by lender")
    ...
    try:
        ...
        if tx_receipt['status'] == 0:
            ...
            print_error("This is a CRITICAL BUG - claimed loans can be repaid!")
            print_error("This would allow borrower to pay after collateral seizure!")
    ...
    if not reverted and (tx_receipt and tx_receipt['status'] == 1):
        ...
        print_error("CRITICAL BUG: Loan in CLAIMED state can be repaid!")
        print_error("This is a serious vulnerability - borrower could pay after losing collateral!")
        sys.exit(1)
    ...
    # Verify loan state unchanged (still CLAIMED)
    ...
    if loan_data_after['initiator'] != loan_data_before['initiator']:
        ...
        sys.exit(1)
    print_success("✓ Loan initiator unchanged (still the seizer)")
    ...
    print_success("TC04: Loan Already Claimed")
    ...
    print(f"\n{CYAN}Summary:{NC}")
    print(f"  - Loans in CLAIMED state cannot be repaid")
    print(f"  - Once collateral is seized, repayment is no longer possible")
    print(f"  - _requireLoanActive validation working correctly")
    print(f"  - Contract prevents post-seizure repayment")
    print(f"  - Loan state remains: CLAIMED")
