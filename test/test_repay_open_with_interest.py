#!/usr/bin/env python3
"""
Test Case TC11: Success - Repay OPEN Loan With Accrued Interest
Objective: Verify successful repayment with significant interest accrued
Tests: Successful repay() execution with meaningful interest calculation
Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction succeeds, loan state → REPAID, interest calculated accurately
"""
...
def main():
    print_section("Test Case TC11: Success - Repay OPEN Loan With Accrued Interest")
    print(f"{CYAN}Objective:{NC} Verify successful repayment with significant interest accrued")
    print(f"{CYAN}Strategy:{NC} Find OPEN loan with elapsed time, verify interest calculation")
    print(f"{CYAN}Expected:{NC} Success, loan state → REPAID, accurate interest\n")
    ...
    # Find an OPEN loan with maximum elapsed time for significant interest
    print_info("\nSearching for OPEN loan with most elapsed time...")
    ...
    print_info(f"  Elapsed blocks since start: {max_elapsed}")
    ...
    print_success("TC11: Success - Repay OPEN Loan With Accrued Interest")
    ...
    print_success("Interest calculation accurate (0 RAO error)")
