#!/usr/bin/env python3
"""
Test Case TC13: Success - Third Party Repays IN_COLLECTION Loan
Objective: Verify successful repayment by third party from IN_COLLECTION state
Tests: Third party can repay from collection, collateral goes to original borrower
Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Success, third party pays, borrower gets collateral, prevents seizure
"""
...
def main():
    print_section("Test Case TC13: Success - Third Party Repays IN_COLLECTION Loan")
    print(f"{CYAN}Objective:{NC} Verify successful third-party repayment from IN_COLLECTION state")
    print(f"{CYAN}Strategy:{NC} Third party pays, collateral to borrower, prevents seizure")
    print(f"{CYAN}Expected:{NC} Success, IN_COLLECTION → REPAID, borrower gets collateral\n")
    ...
    # Use BORROWER2 as third party repayer (not lender, not borrower)
    ...
    # Verify repayer is registered
    repayer_registered = contract.functions.registeredUser(repayer_address).call()
    if not repayer_registered:
        print_error("SETUP ERROR: BORROWER2 (repayer) not registered")
        sys.exit(1)

    # Verify repayer is different from borrower and lender
    if repayer_address.lower() == borrower_address.lower():
        ...
        sys.exit(1)
    if repayer_address.lower() == lender_address.lower():
        print_error("SETUP ERROR: Repayer must be different from lender for this test")
        sys.exit(1)
    print_success(f"✓ Third party repayer (BORROWER2): {repayer_address}")
    print_info(f"  Third party is NOT the borrower or lender (true third-party test)")

    # Find an IN_COLLECTION loan belonging to BORROWER1
    print_info("\nSearching for IN_COLLECTION loan belonging to BORROWER1...")
    ...
    for loan_id in range(next_loan_id):
        try:
            ...
            # Check if loan is IN_COLLECTION and belongs to BORROWER1
            if (loan_data['state'] == STATE_IN_COLLECTION and
                loan_term['borrower'].lower() == borrower_address.lower()):
                test_loan_id = loan_id
                print_success(f"✓ Found IN_COLLECTION loan: Loan ID {loan_id}")
                break
    ...
    if test_loan_id is None:
        print_error("SETUP ERROR: No IN_COLLECTION loan found for BORROWER1")
        print_info("Please create a loan and have lender call collect() first")
        sys.exit(1)
    ...
    # Verify state is IN_COLLECTION
    if loan_data_before['state'] != STATE_IN_COLLECTION:
        print_error(f"ERROR: Loan state is not IN_COLLECTION!")
        sys.exit(1)
    print_success(f"✓ Loan state confirmed: IN_COLLECTION")
    ...
    print(f"  {CYAN}Loan State:{NC} IN_COLLECTION → REPAID")
    print(f"  {CYAN}Repayer:{NC} BORROWER2 (third party)")
    print(f"  {CYAN}TAO Payment:{NC} From BORROWER2 (third party)")
    print(f"  {CYAN}Collateral Return:{NC} To BORROWER1 (original borrower) ← CRITICAL")
    print(f"  {CYAN}Initiator:{NC} BORROWER2 (third party repayer)")
    print(f"  {CYAN}Grace Period:{NC} Repayment prevents seizure")
    print()

    print_info(f"Repaying loan {test_loan_id} as third party...")
    print_info(f"Repayer: {repayer_address} (BORROWER2 - third party)")
    ...
    if loan_data_after['state'] != STATE_REPAID:
        ...
        sys.exit(1)
    print_success("✓ Loan state: IN_COLLECTION → REPAID")

    # CRITICAL: Verify initiator is third party (BORROWER2), not borrower or lender
    if loan_data_after['initiator'].lower() != repayer_address.lower():
        print_error(f"✗ Loan initiator is not third party!")
        print_error(f"  Expected: {repayer_address} (BORROWER2)")
        print_error(f"  Actual: {loan_data_after['initiator']}")
        sys.exit(1)
    print_success("✓ Loan initiator: BORROWER2 (third party repayer)")
    ...
    print_success("TC13: Success - Third Party Repays IN_COLLECTION Loan")
    print_success("Third party repayment from IN_COLLECTION completed successfully")
    print_success("All state validations passed")
    print_success("All balance changes verified")
    print_success("Collateral seizure prevented by third-party repayment")

    print(f"\n{CYAN}Summary:{NC}")
    print(f"  - Loan successfully repaid by third party (BORROWER2)")
    print(f"  - Loan state: IN_COLLECTION → REPAID")
    print(f"  - Third party paid: {actual_repay_amount / 1e9:.9f} TAO")
    print(f"  - Borrower paid: 0 TAO (third party rescued loan)")
    print(f"  - Collateral returned: {loan_term_before['collateralAmount'] / 1e9:.9f} ALPHA → BORROWER1 ← CRITICAL")
    print(f"  - Initiator recorded: BORROWER2 (third party)")
    print(f"  - Interest accrued: {actual_interest / 1e9:.9f} TAO")
    print(f"  - Elapsed blocks: {actual_elapsed}")
    print(f"  - Grace period: Repayment prevented seizure")
    print(f"\n{BOLD}Key Feature Verified:{NC} Third parties can rescue loans in collection, collateral always goes to original borrower!")
