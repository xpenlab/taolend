#!/usr/bin/env python3
"""
Test Case TC02: Loan OPEN (Seize)
Objective: Verify seize fails when loan is OPEN (not IN_COLLECTION)
...
def main():
    print_section("Test Case TC02: Loan OPEN (Seize)")
    print(f"{CYAN}Objective:{NC} Verify seize fails when loan is OPEN (not IN_COLLECTION)")
    print(f"{CYAN}Strategy:{NC} Attempt to seize loan that's not in collection state")
    ...
    # Find a loan in OPEN state
    print_info("Finding a loan in OPEN state...")
    ...
    for test_loan_id in range(next_loan_id - 1, max(0, next_loan_id - 50), -1):
        try:
            ...
            if loan_info and loan_info['data']['state'] == STATE_OPEN:
                ...
                print_success(f"Found loan in OPEN state: ID={loan_id}")
                print_info(f"  Lender: {lender_address}")
                print_info(f"  Borrower: {borrower_address}")
                print_info(f"  State: OPEN (0)")
    ...
    if loan_id is None:
        print_error("No loans found in OPEN state")
        print_error("Please run a borrow test first to create a loan in OPEN state")
        sys.exit(1)
    ...
    print_warning(f"⚠️  Loan is OPEN - cannot seize before collection")
    ...
    print(f"  {CYAN}Reason:{NC} Loan must be IN_COLLECTION before seize")
    ...
    print_warning(f"⚠️  Loan state: OPEN (must collect first, then wait grace period)")
    ...
    print_info(f"Loan ID: {loan_id}")
    print_info(f"State: {loan_data_after['state']} (still OPEN)")
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Seize correctly rejected OPEN loan")
    else:
    ...
    print_success("✓ TC02 PASSED: Seize correctly rejects OPEN loan")
    print_info("State protection: Loan must be IN_COLLECTION before seize")
