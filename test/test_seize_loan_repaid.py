#!/usr/bin/env python3
"""
Test Case TC03: Loan REPAID (Seize)
Objective: Verify seize fails when loan is already repaid
...
def main():
    print_section("Test Case TC03: Loan REPAID (Seize)")
    print(f"{CYAN}Objective:{NC} Verify seize fails when loan is already repaid")
    print(f"{CYAN}Strategy:{NC} Attempt to seize loan in REPAID state")
    ...
    print_warning(f"⚠️  Loan already repaid - cannot seize")
    ...
    print_warning(f"⚠️  Loan state: REPAID (cannot seize repaid loan)")
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Seize correctly rejected REPAID loan")
    else:
    ...
    print_success("✓ TC03 PASSED: Seize correctly rejects REPAID loan")
    print_info("State protection: Cannot seize repaid loans")
