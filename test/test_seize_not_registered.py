...
Objective: Verify seize fails when initiator is not registered
...
def main():
    print_section("Test Case TC01: Initiator Not Registered (Seize)")
    print(f"{CYAN}Objective:{NC} Verify seize fails when initiator is not registered")
    print(f"{CYAN}Strategy:{NC} Attempt seize with unregistered user")
    ...
    # Find an IN_COLLECTION loan that has passed MIN_LOAN_DURATION
    print_info("Finding an eligible IN_COLLECTION loan...")
    ...
    for test_loan_id in range(next_loan_id - 1, max(0, next_loan_id - 50), -1):
        try:
            ...
            if loan_info and loan_info['data']['state'] == STATE_IN_COLLECTION:
                ...
                blocks_passed = current_block - last_update_block

                # Check if MIN_LOAN_DURATION has passed since collect
                if blocks_passed > min_loan_duration:
                    ...
                    print_success(f"Found eligible loan: ID={loan_id}, state=IN_COLLECTION")
                    print_info(f"  Lender: {lender_address}")
                    print_info(f"  Borrower: {borrower_address}")
                    print_info(f"  Last update block: {last_update_block}, Current: {current_block}")
    ...
    if loan_id is None:
        print_error("No eligible IN_COLLECTION loans found that have passed MIN_LOAN_DURATION")
        print_error("Please collect a loan and wait for MIN_LOAN_DURATION blocks")
        sys.exit(1)
    ...
    addresses_list = [
        {"address": unregistered_address, "label": f"UNREGISTERED ({unregistered_label})"},
    ...
    print(f"  {CYAN}Reason:{NC} Initiator not registered (onlyRegistered modifier)")
    print(f"  {CYAN}State Changes:{NC}")
    print(f"    - No state changes (transaction reverts)")
    print(f"    - Only gas deducted from unregistered user's EVM TAO")

    print_info(f"\nAttempting to seize loan {loan_id}...")
    print_warning(f"⚠️  Initiator: {unregistered_address} (UNREGISTERED)")
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Seize correctly rejected unregistered user")
    else:
    ...
    print_success("✓ TC01 PASSED: Seize correctly rejects unregistered user")
