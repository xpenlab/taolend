#!/usr/bin/env python3
"""
Test Case TC09: Too Early (Seize)
Objective: Verify seize fails before MIN_LOAN_DURATION grace period expires
Tests: Duration check - require(block.number > loanData.lastUpdateBlock + MIN_LOAN_DURATION, "too early")
...
def main():
    print_section("Test Case TC09: Too Early (Seize)")
    print(f"{CYAN}Objective:{NC} Verify seize fails before MIN_LOAN_DURATION grace period expires")
    print(f"{CYAN}Strategy:{NC} Lender attempts to seize immediately after collect")
    ...
    print_info(f"MIN_LOAN_DURATION from contract: {min_loan_duration} blocks")

    # Find a loan in IN_COLLECTION state where grace period has NOT passed
    print_info("Finding a loan in IN_COLLECTION state where grace period has NOT passed...")
    ...
    for test_loan_id in range(next_loan_id - 1, max(0, next_loan_id - 100), -1):
        try:
            ...
            if loan_info and loan_info['data']['state'] == STATE_IN_COLLECTION:
                ...
                # Check if grace period has NOT passed (too early to seize)
                if current_block <= seize_block:
                    ...
                    blocks_remaining = seize_block - current_block
                    ...
                    print_info(f"  Seize allowed at block: {seize_block}")
                    print_warning(f"  ⚠️  Grace period remaining: {blocks_remaining} blocks (TOO EARLY)")
                    break
        except Exception as e:
            continue

    if loan_id is None:
        print_error("No loans found in IN_COLLECTION state where grace period has NOT passed")
        print_error("All IN_COLLECTION loans are past the grace period")
        print_error("Please run a collect test first to create a fresh IN_COLLECTION loan")
        sys.exit(1)
    ...
    print_info(f"Current Block: {current_block}")
    print_info(f"Seize Allowed At Block: {seize_block}")
    print_warning(f"⚠️  Grace period remaining: {blocks_remaining} blocks - TOO EARLY to seize")
    ...
    print(f"  {CYAN}Reason:{NC} Grace period (MIN_LOAN_DURATION) has not expired yet")
    ...
    print_warning(f"⚠️  Too early: {blocks_remaining} blocks remaining in grace period")
    ...
    if tx_receipt['status'] == 0:
        ...
        print_success("✓ Test PASSED: Seize correctly rejected attempt before grace period expires")
    else:
    ...
    print_success("✓ TC09 PASSED: Seize correctly rejects attempt before grace period expires")
    print_info("Grace period protection: Borrower has time to repay after collection")
    print_info("Error message: 'too early'")
    print_info(f"Minimum wait: {min_loan_duration} blocks (MIN_LOAN_DURATION)")
