#!/usr/bin/env python3
"""
Test Case TC13: Exceeds Max TAO Amount
Objective: Verify transfer fails when transfer would exceed offer's maxTaoAmount
Tests: _lenderBalanceChecker - require(userLendBalance[_offer.lender][_offer.offerId] + _taoAmount <= _offer.maxTaoAmount, "exceeds max")
...
def main():
    """Test transfer fails when transfer would exceed offer's maxTaoAmount"""

    print_section("Test Case TC13: Exceeds Max TAO Amount")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify transfer fails when transfer would exceed offer's maxTaoAmount")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} NEW_LENDER's offer has maxTaoAmount < repayAmount")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction reverts with 'exceeds max'")
    ...
    new_lender_address = addresses['LENDER2']['evmAddress']  # NEW_LENDER with offer maxTaoAmount = 10 TAO
    ...
    # Load new offer from LENDER2 with small maxTaoAmount
    print_info("\nLoading offer from NEW_LENDER (LENDER2) with small maxTaoAmount...")
    new_offer_file = "offers/4fd4d47971c4c6cde5d92518fc661764a51002380734063567e8304299abb26f.json"  # LENDER2's offer with maxTaoAmount=10
    new_offer = load_offer_from_file(new_offer_file)

    offer_netuid = new_offer['netuid']
    new_offer_lender = new_offer['lender']
    offer_max_tao = new_offer['maxTaoAmount']
    ...
    print_info(f"  Max TAO Amount: {offer_max_tao / 1e9:.2f} TAO")
    ...
    # Check if repayAmount exceeds maxTaoAmount
    print_info(f"\nMax TAO Check:")
    print_info(f"  Offer maxTaoAmount: {offer_max_tao / 1e9:.2f} TAO")
    print_info(f"  Required repayAmount: {repay_amount / 1e9:.9f} TAO")

    if repay_amount <= offer_max_tao:
        print_warning(f"⚠ repayAmount ({repay_amount / 1e9:.9f}) <= maxTaoAmount ({offer_max_tao / 1e9:.2f})")
        print_warning("This test requires repayAmount > maxTaoAmount. Test may not demonstrate expected behavior.")
    else:
        print_success(f"✓ repayAmount ({repay_amount / 1e9:.9f}) > maxTaoAmount ({offer_max_tao / 1e9:.2f})")
    ...
    print_info(f"  NEW_LENDER userLendBalance (this offer): {new_lender_balance_before / 1e9:.2f} TAO")
    ...
    print(f"  {Colors.RED}Revert:{Colors.ENDC} 'exceeds max'")
    print(f"  {Colors.CYAN}Reason:{Colors.ENDC} repayAmount ({repay_amount / 1e9:.9f} TAO) > maxTaoAmount ({offer_max_tao / 1e9:.2f} TAO)")
    ...
    print_success("TC13: Exceeds Max TAO Amount")
    print_success("Transaction correctly reverted with 'exceeds max'")
    ...
    print(f"  - Cannot transfer loan when repayAmount exceeds offer's maxTaoAmount")
    print(f"  - Max TAO validation working correctly")
    print(f"  - Offer maxTaoAmount: {offer_max_tao / 1e9:.2f} TAO")
    print(f"  - Required repayAmount: {repay_amount / 1e9:.9f} TAO")
    print(f"  - Excess: {(repay_amount - offer_max_tao) / 1e9:.9f} TAO")
