#!/usr/bin/env python3
"""
Test Case TC17: Original Lender, IN_COLLECTION State, Success
Objective: Verify successful transfer by original lender from IN_COLLECTION state
Tests: Complete transfer flow from IN_COLLECTION with all state changes

Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction succeeds, loan transferred to new lender, no MIN_LOAN_DURATION check
"""
...
def main():
    """Test successful transfer by original lender from IN_COLLECTION state"""

    print_section("Test Case TC17: Original Lender, IN_COLLECTION State, Success")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify successful transfer by original lender from IN_COLLECTION state")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} OLD_LENDER collects first, then transfers to NEW_LENDER")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction succeeds, no MIN_LOAN_DURATION check for IN_COLLECTION")
    ...
    # If loan is OPEN, need to collect first
    if current_state == STATE_OPEN:
        print_warning("Loan is in OPEN state, need to call collect() first")
        ...
        # Call collect()
        print_info(f"\nCalling collect() to move loan to IN_COLLECTION state...")
    ...
    print(f"  {Colors.GREEN}Success:{Colors.ENDC} Transfer completes successfully from IN_COLLECTION")
    print(f"  {Colors.CYAN}Note:{Colors.ENDC} No MIN_LOAN_DURATION check for IN_COLLECTION state")
    ...
    print_success("TC17: Original Lender, IN_COLLECTION State, Success")
    print_success("Transfer from IN_COLLECTION completed successfully")
    ...
    print(f"  - No MIN_LOAN_DURATION check for IN_COLLECTION state ✓")
