#!/usr/bin/env python3
"""
Test Case TC16: Original Lender, OPEN State, Success
Objective: Verify successful transfer by original lender from OPEN state after MIN_LOAN_DURATION
Tests: Complete transfer flow with all state changes and balance updates

Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction succeeds, loan transferred to new lender
"""
...
def main():
    """Test successful transfer by original lender from OPEN state"""

    print_section("Test Case TC16: Original Lender, OPEN State, Success")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify successful transfer by original lender from OPEN state")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} OLD_LENDER transfers OPEN loan to NEW_LENDER after MIN_LOAN_DURATION")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction succeeds, loan transferred with correct state changes")
    ...
    # Test accounts - Dynamically determine based on current lender
    loan_id = 9
    ...
    if old_lender_from_loan.lower() == addresses['LENDER1']['evmAddress'].lower():
        ...
        initiator_address = addresses['LENDER1']['evmAddress']
        ...
        initiator_address = addresses['LENDER2']['evmAddress']
        initiator_private_key = os.environ.get("LENDER2_PRIVATE_KEY") or os.environ.get("ETH_PRIVATE_KEY")
    ...
    # Verify accounts are registered (after determining who they are)
    ...
    if not is_registered:
        ...
        return 1
    print_success(f"✓ INITIATOR (OLD_LENDER) registered: {initiator_address}")
    ...
    MIN_LOAN_DURATION = contract.functions.MIN_LOAN_DURATION().call()  # Read from contract
    ...
    # Verify initiator is old lender
    if initiator_address.lower() != old_lender_from_loan.lower():
        print_error(f"⚠ Test setup error: Initiator ({initiator_address}) is not old lender ({old_lender_from_loan})")
        return 1

    print_success(f"✓ Initiator is old lender")

    # Load new offer (already set above based on current lender)
    print_info(f"\nLoading new offer...")
    ...
    print(f"  {Colors.GREEN}Success:{Colors.ENDC} Transfer completes successfully")
    ...
    print(f"    - NEW_LENDER contract TAO[0]: -{repay_amount / 1e9:.9f} TAO")
    print(f"    - NEW_LENDER userLendBalance: +{repay_amount / 1e9:.9f} TAO")
    ...
    # loanRecords returns: [loanDataId, offerId, startBlock, loanAmount, initiator, state, repayBlock]
    ...
    # Verify new loan amount equals actual repay amount (calculated with precise block)
    ...
    print_info("\nExpected changes:")
    print_info(f"  - OLD_LENDER contract TAO[0]: +{(repay_amount - protocol_fee) / 1e9:.9f} TAO")
    print_info(f"  - NEW_LENDER contract TAO[0]: -{repay_amount / 1e9:.9f} TAO")
    print_info(f"  - Protocol fee: +{protocol_fee / 1e9:.9f} TAO")
    print_info(f"  - OLD_LENDER EVM TAO: decreased by gas cost")
    ...
    print_success("TC16: Original Lender, OPEN State, Success")
    ...
    print(f"  - Loan successfully transferred from OLD_LENDER to NEW_LENDER")
