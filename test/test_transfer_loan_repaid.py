...
Objective: Verify transfer fails when loan is in REPAID state
...
def main():
    """Test transfer fails when loan is in REPAID state"""

    print_section("Test Case TC02: Loan Already Repaid")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify transfer fails when loan is in REPAID state")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} Attempt transfer on repaid loan")
    ...
    print_info(f"Using Loan ID 8 (should be in REPAID state)")
    ...
    # Find loan in REPAID state (we just repaid Loan ID 8)
    loan_id = 8
    ...
    if loan_data['state'] != STATE_REPAID:
        print_error(f"Loan {loan_id} is not in REPAID state (current: {STATE_NAMES.get(loan_data['state'], 'UNKNOWN')})")
        print_info("Please repay the loan first using: python3 scripts/cli.py repay --account BORROWER1 --loan-id 8")
        return 1

    print_success(f"✓ Found loan in REPAID state: Loan ID {loan_id}")
    ...
    print(f"  {Colors.CYAN}Reason:{Colors.ENDC} Loan is in REPAID state (_requireLoanActive)")
    ...
    print_success("TC02: Loan Already Repaid")
    ...
    print(f"  - Cannot transfer loan in REPAID state")
