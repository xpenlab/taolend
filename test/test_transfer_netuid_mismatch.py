#!/usr/bin/env python3
"""
Test Case TC07: Netuid Mismatch
Objective: Verify transfer fails when offer netuid doesn't match loan netuid
Tests: require(term.netuid == _offer.netuid, "netuid mismatch")
...
def main():
    """Test transfer fails when offer netuid doesn't match loan netuid"""

    print_section("Test Case TC07: Netuid Mismatch")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify transfer fails when offer netuid doesn't match loan netuid")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} Attempt transfer with netuid mismatch")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction reverts with 'netuid mismatch'")
    ...
    # Load new offer from NEW_LENDER with DIFFERENT netuid
    print_info("\nLoading new offer with DIFFERENT netuid...")
    # Offer for netuid 2 (loan is on netuid 3)
    new_offer_file = "offers/3232b5e5225f09299e3aadbf2d93f643514b40aff87e92d49d6070049a6457f2.json"
    ...
    if offer_netuid == loan_netuid:
        print_error(f"⚠ Test setup error: Offer netuid ({offer_netuid}) matches loan netuid ({loan_netuid})")
        print_error("This test requires netuid mismatch")
        return 1

    print_warning(f"✓ Netuid mismatch confirmed: Loan netuid={loan_netuid}, Offer netuid={offer_netuid}")
    ...
    test_netuids = [0, 2, 3]  # Include both netuids
    ...
    print(f"  {Colors.RED}Revert:{Colors.ENDC} 'netuid mismatch'")
    print(f"  {Colors.CYAN}Reason:{Colors.ENDC} Loan netuid ({loan_netuid}) != Offer netuid ({offer_netuid})")
    ...
    print_info(f"Loan netuid: {loan_netuid}, Offer netuid: {offer_netuid}")
    ...
    print_success("TC07: Netuid Mismatch")
    print_success("Transaction correctly reverted with 'netuid mismatch'")
    ...
    print(f"  - Cannot transfer loan with netuid mismatch")
    print(f"  - Netuid validation working correctly")
    print(f"  - Contract state protected from invalid operations")
    print(f"  - Loan netuid: {loan_term_after['netuid']}")
    print(f"  - Offer netuid: {offer_netuid}")
