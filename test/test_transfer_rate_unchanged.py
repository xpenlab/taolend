#!/usr/bin/env python3
"""
Test Case TC21: Transfer With Rate Unchanged
Objective: Verify successful transfer with same rate as original loan
Tests: Rate validation with unchanged rate (e.g., 0.6% -> 0.6%)

Strategy:
1. Use existing loan with specific rate (e.g., 0.6%)
2. Transfer to offer with same rate (0.6%)
3. Verify rate unchanged
4. Verify all balance changes correct

Expected: Transaction succeeds, rate remains same
"""
...
def main():
    """Test transfer with rate unchanged"""

    print_section("Test Case TC21: Transfer With Rate Unchanged")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify successful transfer with same rate as original loan")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} Transfer loan to new lender with identical rate")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction succeeds, rate remains unchanged")
    ...
    print_info("This test requires a loan with a specific rate")
    print_info("Example: Loan 10 (0.6% rate)")
    ...
    if old_lender_address.lower() == addresses['LENDER1']['evmAddress'].lower():
        ...
        new_offer_file = "offers/1408429de5e79990088998f8a9d162059901040e433a0ec7a05622db6880c9f5.json"
    ...
    # Load new offer with same rate
    print_info(f"\nLoading new offer with same rate...")
    ...
    if offer_new['dailyInterestRate'] == offer_old['dailyInterestRate']:
        print_success(f"✓ New rate is IDENTICAL to old rate (rate unchanged)")
        print_success(f"✓ This tests the rate validation logic for unchanged rates")
    elif offer_new['dailyInterestRate'] < offer_old['dailyInterestRate']:
        print_warning(f"⚠ New rate is LOWER than old rate (rate decreased)")
        print_warning(f"⚠ This test is designed for rate unchanged scenarios")
    else:
        print_warning(f"⚠ New rate is HIGHER than old rate (rate increased)")
        print_warning(f"⚠ This test is designed for rate unchanged scenarios")
    ...
    print_info(f"Transferring Loan {loan_id} with same rate...")
    ...
    # ========================================================================
    # CRITICAL: Verify Rate Unchanged
    # ========================================================================
    print_info(f"\n{'='*80}")
    print_info(f"{Colors.BOLD}CRITICAL VERIFICATION: Rate Unchanged{Colors.ENDC}")
    print_info(f"{'='*80}")
    ...
    if new_rate == old_rate:
        print_success(f"✓ RATE UNCHANGED (IDENTICAL)")
        print_success(f"✓ {new_rate} == {old_rate}")
        print_info(f"\nBorrower Impact:")
        print_info(f"  Daily interest rate: Remains {new_rate / 1e9 * 100:.2f}%")
        print_info(f"  No change in interest cost")
    elif new_rate < old_rate:
        print_warning(f"⚠ Rate decreased (not unchanged)")
    else:
        print_warning(f"⚠ Rate increased (not unchanged)")
    ...
    # Final Summary
    # ========================================================================
    print_section("Test Result: TC21 - PASSED")
    print_success("✓ Transfer with rate unchanged succeeded")
    ...
    print_success(f"✓ Borrower continues with same interest rate")
