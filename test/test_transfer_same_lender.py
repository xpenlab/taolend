#!/usr/bin/env python3
"""
Test Case TC08: Same Lender
Objective: Verify transfer fails when new lender is same as old lender
Tests: require(oldOffer.lender != _offer.lender, "same lender")

Strategy: 8-step testing pattern with BalanceChecker and get_loan_full
Expected: Transaction reverts with "same lender"
"""
...
def main():
    """Test transfer fails when new lender is same as old lender"""

    print_section("Test Case TC08: Same Lender")
    print(f"{Colors.CYAN}Objective:{Colors.ENDC} Verify transfer fails when new lender is same as old lender")
    print(f"{Colors.CYAN}Strategy:{Colors.ENDC} Attempt transfer with same lender")
    print(f"{Colors.CYAN}Expected:{Colors.ENDC} Transaction reverts with 'same lender'")
    ...
    # Load new offer from SAME LENDER (should match old lender)
    print_info("\nLoading offer from SAME lender...")
    # Use LENDER1's offer for netuid 3 (same as loan's lender)
    new_offer_file = "offers/959ef93720143532e0192a296ede97d4377c39e68a49865638382a6b8575295d.json"  # LENDER1's offer for netuid 3
    ...
    # Verify lender is the same
    if new_offer_lender.lower() != old_lender_from_loan.lower():
        print_error(f"⚠ Test setup error: New lender ({new_offer_lender}) is not the same as old lender ({old_lender_from_loan})")
        print_error("This test requires same lender")
        return 1

    print_warning(f"✓ Same lender confirmed: Old lender = New lender = {new_offer_lender}")
    ...
    print(f"  {Colors.RED}Revert:{Colors.ENDC} 'same lender'")
    print(f"  {Colors.CYAN}Reason:{Colors.ENDC} New lender ({new_offer_lender}) is same as old lender ({old_lender_from_loan})")
    ...
    print_success("TC08: Same Lender")
    print_success("Transaction correctly reverted with 'same lender'")
    ...
    print(f"  - Cannot transfer loan to same lender")
    print(f"  - Lender validation working correctly")
    print(f"  - Contract state protected from invalid operations")
    print(f"  - Old lender: {offer_before['lender']}")
    print(f"  - New lender (rejected): {new_offer_lender}")
