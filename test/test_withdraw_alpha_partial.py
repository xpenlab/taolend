#!/usr/bin/env python3
"""
Test Case TC28: withdrawAlpha() - Partial Withdrawal
Objective: Verify withdrawAlpha works correctly when withdrawing partial balance
Tests: Withdraw 50% of balance and verify remaining balance is correct

Strategy: 8-step testing pattern with BalanceChecker
Expected: Transaction succeeds, partial amount withdrawn, remaining balance correct
"""
...
def main():
    print_section("Test Case TC28: withdrawAlpha() - Partial Withdrawal")
    print(f"{CYAN}Objective:{NC} Verify withdrawAlpha works correctly when withdrawing partial balance")
    print(f"{CYAN}Strategy:{NC} Withdraw 50% of balance and verify remaining balance")
    print(f"{CYAN}Expected:{NC} Transaction succeeds, partial withdrawal, remaining balance correct\n")
    ...
    # Check user balance and calculate 50%
    ...
    withdraw_amount_rao = user_balance // 2
    ...
    print_info(f"  Withdraw Amount: {withdraw_amount_rao / 1e9:.2f} ALPHA (50%)")
    print_info(f"  Expected Remaining: {(user_balance - withdraw_amount_rao) / 1e9:.2f} ALPHA")
    ...
    # ========================================================================
    print_section("Step 4: Execute withdrawAlpha() - Partial Withdrawal")
    ...
    print(f"    - Remaining balance: {(user_contract_balance_before - withdraw_amount_rao) / 1e9:.2f} ALPHA (50%)")
    print()

    print_info(f"Withdrawing {withdraw_amount_rao / 1e9:.2f} ALPHA (50% of balance) from netuid {test_netuid}...")
    ...
    # 1. Verify user balance decreased by 50%
    ...
    if user_contract_balance_after != expected_user_balance:
        ...
        print_success(f"✓ Remaining balance is 50%: {user_contract_balance_after / 1e9:.2f} ALPHA")
    ...
    # 3. Verify remaining balance > 0 (not full withdrawal)
    if user_contract_balance_after == 0:
        print_error(f"✗ User balance is zero (expected partial withdrawal)!")
        all_checks_passed = False
    else:
        print_success(f"✓ User still has remaining balance: {user_contract_balance_after / 1e9:.2f} ALPHA")
    ...
    if all_checks_passed:
        print_success("✓✓✓ TC28 TEST PASSED ✓✓✓")
        print_success("withdrawAlpha() partial withdrawal executed successfully")
        print_success("All state validations passed")

        print(f"\n{CYAN}Summary:{NC}")
        print(f"  - Withdrew {withdraw_amount_rao / 1e9:.2f} ALPHA (50%) from netuid {test_netuid}")
        print(f"  - User balance: {user_contract_balance_before / 1e9:.2f} → {user_contract_balance_after / 1e9:.2f} ALPHA")
        print(f"  - Remaining: {user_contract_balance_after / 1e9:.2f} ALPHA (50%)")
        print(f"  - Subnet balance: {subnet_balance_before / 1e9:.2f} → {subnet_balance_after / 1e9:.2f} ALPHA")
        print(f"  - Gas used: {tx_receipt['gasUsed']}")

        return 0
    else:
        print_error("✗✗✗ TC28 TEST FAILED ✗✗✗")
        return 1
