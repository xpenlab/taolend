#!/usr/bin/env python3
"""
Test Case TC09: withdrawTao() - Full Withdrawal
Objective: Verify withdrawTao works correctly when withdrawing entire balance
Tests: Withdraw 100% of user's TAO balance

Strategy: 8-step testing pattern with BalanceChecker
Expected: Transaction succeeds, user balance becomes zero
"""
...
def main():
    print_section("Test Case TC09: withdrawTao() - Full Withdrawal")
    print(f"{CYAN}Objective:{NC} Verify withdrawTao works correctly when withdrawing entire balance")
    print(f"{CYAN}Strategy:{NC} Withdraw 100% of user's TAO balance and verify state changes")
    print(f"{CYAN}Expected:{NC} Transaction succeeds, user balance becomes zero\n")
    ...
    withdraw_amount_rao = user_contract_balance

    print_success(f"✓ User has contract balance: {user_contract_balance / 1e9:.2f} TAO")
    print_info(f"  Withdrawing 100%: {withdraw_amount_rao / 1e9:.2f} TAO")
    ...
    print(f"    - User's contract balance (netuid=0): {user_contract_balance / 1e9:.2f} → 0.00 TAO")
    print(f"    - User withdraws 100%: {withdraw_amount_rao / 1e9:.2f} TAO")
    print(f"    - User retains: 0.00 TAO")
    print()

    print_info(f"Withdrawing {withdraw_amount_rao / 1e9:.2f} TAO (100% of balance)...")
    ...
    # 3. Verify user contract balance is exactly zero
    ...
    if user_contract_balance_after != 0:
        print_error(f"✗ User contract balance should be zero!")
        print_error(f"  Expected: 0.00 TAO")
        print_error(f"  Actual:   {user_contract_balance_after / 1e9:.2f} TAO")
        all_checks_passed = False
    else:
        print_success(f"✓ User contract balance is exactly zero")

    # 4. Verify subnet balance decreased by exact amount
    ...
    if all_checks_passed:
        print_success("✓✓✓ TC09 TEST PASSED ✓✓✓")
        print_success("withdrawTao() full withdrawal successful")
        print_success("User correctly withdrew entire balance")

        print(f"\n{CYAN}Summary:{NC}")
        print(f"  - Original balance: {user_contract_balance / 1e9:.2f} TAO")
        print(f"  - Withdrawn (100%): {withdraw_amount_rao / 1e9:.2f} TAO")
        print(f"  - Remaining: {user_contract_balance_after / 1e9:.2f} TAO")
        print(f"  - Gas used: {tx_receipt['gasUsed']} units")

        return 0
    else:
        print_error("✗✗✗ TC09 TEST FAILED ✗✗✗")
