#!/usr/bin/env python3
"""
Test Case TC12: withdrawTao() - Low Balance
Objective: Verify withdrawTao reverts when user has insufficient balance
...
def main():
    print_section("Test Case TC12: withdrawTao() - Low Balance")
    print(f"{CYAN}Objective:{NC} Verify withdrawTao reverts when user has insufficient balance")
    ...
    # Get current balance and attempt to withdraw more
    user_contract_balance = contract.functions.userAlphaBalance(registered_address, 0).call()
    withdraw_amount_rao = user_contract_balance + (10 * 10**9)  # Balance + 10 TAO (more than available)
    ...
    print_info(f"  Shortfall: {(withdraw_amount_rao - user_contract_balance) / 1e9:.2f} TAO")
    ...
    print_info(f"Attempting to withdraw {withdraw_amount_rao / 1e9:.2f} TAO (more than balance)...")
    print_info(f"User: {registered_address} ({registered_label}, registered)")
    print_info(f"User balance: {user_contract_balance / 1e9:.2f} TAO")
    ...
    if all_checks_passed:
        print_success("✓✓✓ TC12 TEST PASSED ✓✓✓")
        print_success("withdrawTao() correctly rejects withdrawal exceeding balance")
        print_success("Balance check (line 640) working as expected")

        print(f"\n{CYAN}Summary:{NC}")
        print(f"  - User: {registered_label} (registered)")
        print(f"  - User balance: {user_contract_balance / 1e9:.2f} TAO")
        print(f"  - Attempted withdrawal: {withdraw_amount_rao / 1e9:.2f} TAO")
        print(f"  - Shortfall: {(withdraw_amount_rao - user_contract_balance) / 1e9:.2f} TAO")
