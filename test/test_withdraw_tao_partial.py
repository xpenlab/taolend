#!/usr/bin/env python3
"""
Test Case TC08: withdrawTao() - Partial Withdrawal
Objective: Verify withdrawTao works correctly when withdrawing part of balance
Tests: Withdraw 50% of user's TAO balance

Strategy: 8-step testing pattern with BalanceChecker
Expected: Transaction succeeds, user retains 50% of balance
"""
...
def main():
    print_section("Test Case TC08: withdrawTao() - Partial Withdrawal")
    print(f"{CYAN}Objective:{NC} Verify withdrawTao works correctly when withdrawing part of balance")
    print(f"{CYAN}Strategy:{NC} Withdraw 50% of user's TAO balance and verify state changes")
    print(f"{CYAN}Expected:{NC} Transaction succeeds, user retains 50% of balance\n")
    ...
    if user_contract_balance == 0:
        print_error(f"SETUP ERROR: User has zero balance")
    ...
    # Calculate 50% withdrawal
    withdraw_amount_rao = user_contract_balance // 2  # 50% of balance

    if withdraw_amount_rao == 0:
        print_error(f"SETUP ERROR: Balance too small for partial withdrawal")
        print_error(f"  Current balance: {user_contract_balance / 1e9:.2f} TAO")
        sys.exit(1)

    print_success(f"✓ User has contract balance: {user_contract_balance / 1e9:.2f} TAO")
    print_info(f"  Withdrawing 50%: {withdraw_amount_rao / 1e9:.2f} TAO")
    ...
    remaining_balance = user_contract_balance - withdraw_amount_rao

    print(f"\n{BOLD}Expected Result:{NC}")
    print(f"  {GREEN}Success:{NC} Transaction succeeds")
    print(f"  {CYAN}State Changes:{NC}")
    print(f"    - User's contract balance (netuid=0): {user_contract_balance / 1e9:.2f} → {remaining_balance / 1e9:.2f} TAO")
    print(f"    - User withdraws 50%: {withdraw_amount_rao / 1e9:.2f} TAO")
    print(f"    - User retains 50%: {remaining_balance / 1e9:.2f} TAO")
    print()

    print_info(f"Withdrawing {withdraw_amount_rao / 1e9:.2f} TAO (50% of balance)...")
    ...
    # 3. Verify user contract balance is exactly 50% of original
    ...
    if user_contract_balance_after != remaining_balance:
        print_error(f"✗ User contract balance incorrect!")
        print_error(f"  Expected: {remaining_balance / 1e9:.2f} TAO (50% remaining)")
        print_error(f"  Actual:   {user_contract_balance_after / 1e9:.2f} TAO")
        all_checks_passed = False
    else:
        print_success(f"✓ User contract balance is exactly 50% of original: {user_contract_balance_after / 1e9:.2f} TAO")
    ...
    if all_checks_passed:
        print_success("✓✓✓ TC08 TEST PASSED ✓✓✓")
        print_success("withdrawTao() partial withdrawal successful")
        print_success("User correctly withdrew 50% of balance")

        print(f"\n{CYAN}Summary:{NC}")
        print(f"  - Original balance: {user_contract_balance / 1e9:.2f} TAO")
        print(f"  - Withdrawn (50%): {withdraw_amount_rao / 1e9:.2f} TAO")
        print(f"  - Remaining (50%): {user_contract_balance_after / 1e9:.2f} TAO")
        print(f"  - Gas used: {tx_receipt['gasUsed']} units")

        return 0
    else:
        print_error("✗✗✗ TC08 TEST FAILED ✗✗✗")
